...
(function taskrunner() {
    ...
    var order      = [
        ...
        longname   = 0,
        namepad   = function taskrunner_namepad(name) {
            var a = name.length;
        ...
        next       = function taskrunner_next() {
            console.log("");
            if (order.length < 1) {
                return humantime(true);
            }
            phases[order[0]]();
            order.splice(0, 1);
        };
    phases.coreunits       = function taskrunner_coreunits() {
        var raw     = [],
            correct = [],
            countr  = 0,
            countc  = 0,
    ...
    phases.diffunits       = function taskrunner_diffunits() {
        return;
    };
    phases.lint            = function taskrunner_lint() {
        var ignoreDirectory = [
                ".vscode",
                "ace",
                "bin",
    ...
    phases.moduleInstall   = function taskrunner_moduleInstall() {
        var dateobj  = new Date(),
            ...
            handler   = function taskrunner_moduleInstall_handler() {
                ...
                fs
                    .stat(modules[mod].dir, function taskrunner_moduleInstall_handler_stat(erstat, stats) {
                        var add = function taskrunner_moduleInstall_handler_stat_add() {
                            console.log("Adding " + modules[mod].name);
                            child("git submodule add " + modules[mod].repo, function taskrunner_moduleInstall_handler_stat_add_submodule(era, stdouta, stdoutera) {
                                ...
                                child("git clone " + modules[mod].repo, function taskrunner_moduleInstall_handler_stat_add_submodule_clone(erb, stdoutb, stdouterb) {
                                    if (erb !== null) {
                                        errout(erb);
                                    }
                        ...
                        if (stats.isDirectory() === true) {
                            return fs
                                .readdir(modules[mod].dir, function taskrunner_moduleInstall_handler_stat_readdir(direrr) {
                                    if (direrr !== null) {
                                        errout(direrr);
                                    }
                                    ind += 1;
        ...
        editions = function taskrunner_moduleInstall_editions(appName, cloned) {
            var modout = function taskrunner_moduleInstall_editions_modout() {
                    var x   = 0,
                        len = keys.length;
                ...
                submod = function taskrunner_moduleInstall_editions_submod(output) {
                    ...
                    if (appName === "jslint") {
                        fs
                            .readFile(appFile, "utf8", function taskrunner_moduleInstall_editions_submod_lintread(erread, data) {
                                ...
                                if (data.slice(data.length - 30).indexOf("\nmodule.exports = jslint;") < 0) {
                                    ...
                                    fs
                                        .writeFile(appFile, data, "utf8", function taskrunner_moduleInstall_editions_submod_lintread_lintwrite(erwrite) {
                ...
                each   = function taskrunner_moduleInstall_editions_each(val, idx) {
                    appName = val;
                    ind     = idx + 1;
                    submod(false);
                },
                update = function taskrunner_moduleInstall_editions_update() {
                    child("git submodule update", function taskrunner_moduleInstall_editions_update_child(erd, stdoutd, stdouterd) {
                        if (erd !== null) {
                            errout(erd);
                        }
            ...
            if (ind === keys.length) {
                if (today !== date) {
                    child("git checkout jslint.js", {
                        ...
                        if (cloned === true) {
                            child("git submodule init", function taskrunner_moduleInstall_editions_checkoutJSLint_init(erc, stdoutc, stdouterc) {
                                if (erc !== null) {
                                    errout(erc);
                                }
    ...
    phases.packagejson = function taskrunner_packagejson() {
        fs
    ...
    phases.simulations = function taskrunner_simulations() {
        var passcount = [], //passing tests in local group
            ...
            shell     = function taskrunner_simulations_shell(testData) {
                var tab       = (function taskrunner_simulations_shell_child_writeLine_tab() {
                    ...
                    childsim  = function taskrunner_simulations_shell_childsim(param) {
                                            ...
                                            if (depth > -1) {
                                                ...
                                                if (finished[depth] < grouplen[depth]) {
                                                    ...
                                                    if (units[depth][index].group !== undefined) {
                                                        ...
                                                        taskrunner_simulations_shell_childsim(units[depth][index]);
                                                    }
                                                }
                                            } else {
                                                console.log("");
                                                    ...
                                                    var execCallback = function taskrunner_simulations_shell_child_writeLine_teardown_task_exec(err, stdout, stderr) {
                                                        ...
                                                        if (typeof err === "string") {
                                                            ...
                                                            if (err.indexOf("The directory is not empty.") > 0) {
                                                                ...
                                                                a -= 1;
                                                                return setTimeout(child(tasks[a], taskrunner_simulations_shell_child_writeLine_teardown_task_exec), 1000);
                                                            }
                                        ...
                                        if (a < len) {
                                            ...
                                            for (index[depth] = index[depth]; index[depth] < grouplen[depth]; index[depth] += 1) {
                                                if (units[depth][index[depth]].group === undefined) {
                                                    childsim(units[depth][index[depth]]);
