            ...
            lint       : function taskrunner_lint() {
                ...
                (function taskrunner_lint_install() {
                    ...
                    fs.stat("JSLint", function taskrunner_lint_install_stat(erstat, stats) {
                        ...
                        if (erstat !== null && erstat !== undefined && erstat.toString().indexOf("no such file or directory") < 0) {
                            return errout(erstat);
                        }
                        if (stats !== undefined && stats.isDirectory() === true) {
                                    ...
                                    units: [
                                        {
                                            ...
                                            verify: "<?xml version=\"1.0\" encoding=\"UTF-8\" ?><!DOCTYPE html PUBLIC \"-//W3C//DTD" +
                                                        ...
                                                        "),listLen=listnodes.length;for(a=0;a<listLen;a+=1){lists.push(listnodes[a].get" +
                                                        "ElementsByTagName(\"li\"))}max=(max>=lists[0].length)?lists[0].length:max;if(i" +
                                                        "nner.charAt(0)===\"-\"){self.innerHTML=\"+\"+inner.substr(1);for(a=min;a<max;a" +
                                                        "+=1){for(b=0;b<listLen;b+=1){lists[b][a].style.display=\"none\"}}}else{self.in" +
                                                        "nerHTML=\"-\"+inner.substr(1);for(a=min;a<max;a+=1){for(b=0;b<listLen;b+=1){li" +
                                                        "sts[b][a].style.display=\"block\"}}}};pd.colSliderGrab=function(e){\"use stric" +
                                                        "t\";var event=e||window.event,touch=(e!==null&&e.type===\"touchstart\"),node=t" +
                                                        "his,diffRight=node.parentNode,diff=diffRight.parentNode,subOffset=0,lists=diff" +
                                                        ".getElementsByTagName(\"ol\"),counter=lists[0].clientWidth,data=lists[1].clien" +
                                                        "tWidth,width=lists[2].parentNode.clientWidth,total=lists[2].parentNode.parentN" +
                                                        "ode.clientWidth,offset=lists[2].parentNode.offsetLeft-lists[2].parentNode.pare" +
                                                        "ntNode.offsetLeft,min=((total-counter-data-2)-width),max=(total-width-counter)" +
                                                        ",status=\"ew\",minAdjust=min+15,maxAdjust=max-15,withinRange=false,diffLeft=di" +
                                                        "ffRight.previousSibling,drop=function dom__event_colSliderGrab_drop(f){f=f||wi" +
                                                        "ndow.event;f.preventDefault();node.style.cursor=status+\"-resize\";if(touch===" +
                                                        "true){document.ontouchmove=null;document.ontouchend=null}else{document.onmouse" +
                                                        "move=null;document.onmouseup=null}},boxmove=function dom__event_colSliderGrab_" +
                                                        "boxmove(f){f=f||window.event;f.preventDefault();if(touch===true){subOffset=off" +
                                                        "set-f.touches[0].clientX}else{subOffset=offset-f.clientX}if(subOffset>minAdjus" +
                                                        "t&&subOffset<maxAdjust){withinRange=true}if(withinRange===true&&subOffset>maxA" +
                                                        "djust){diffRight.style.width=((total-counter-2)/10)+\"em\";status=\"e\"}else i" +
                                                        "f(withinRange===true&&subOffset<minAdjust){diffRight.style.width=((total-count" +
                                                        "er-data-2)/10)+\"em\";status=\"w\"}else if(subOffset<max&&subOffset>min){diffR" +
                                                        "ight.style.width=((width+subOffset)/10)+\"em\";status=\"ew\"}if(touch===true){" +
                                                        "document.ontouchend=drop}else{document.onmouseup=drop}};event.preventDefault()" +
                                                        ";if(typeof pd.data===\"object\"&&pd.data.node.report.code.box!==null){offset+=" +
                                                        "pd.data.node.report.code.box.offsetLeft;offset-=pd.data.node.report.code.body." +
                                                        "scrollLeft}else{subOffset=(document.body.parentNode.scrollLeft>document.body.s" +
                                                        "crollLeft)?document.body.parentNode.scrollLeft:document.body.scrollLeft;offset" +
                                                        "-=subOffset}offset+=node.clientWidth;node.style.cursor=\"ew-resize\";diff.styl" +
                                                        "e.width=(total/10)+\"em\";diff.style.display=\"inline-block\";if(diffLeft.node" +
                                                        "Type!==1){do{diffLeft=diffLeft.previousSibling}while(diffLeft.nodeType!==1)}di" +
                                                        "ffLeft.style.display=\"block\";diffRight.style.width=(diffRight.clientWidth/10" +
                                                        ")+\"em\";diffRight.style.position=\"absolute\";if(touch===true){document.ontou" +
                                                        "chmove=boxmove;document.ontouchstart=false}else{document.onmousemove=boxmove;d" +
                                                        "ocument.onmousedown=null}return false};(function(){\"use strict\";var lists=do" +
                                                        "cument.getElementById(\"prettydiff\").getElementsByTagName(\"ol\"),cells=lists" +
                                                        "[0].getElementsByTagName(\"li\"),len=cells.length,a=0;for(a=0;a<len;a+=1){if(c" +
                                                        "ells[a].getAttribute(\"class\")===\"fold\"){cells[a].onclick=pd.difffold}}if(l" +
                                                        "ists.length>3){lists[2].onmousedown=pd.colSliderGrab;lists[2].ontouchstart=pd." +
                                                        "colSliderGrab}pd.colorscheme=document.getElementById(\"colorScheme\");pd.color" +
                                                        "scheme.onchange=pd.colorchange}());//]]>\r\n</script></body></html>"
                                        }
                                    ]
                                }, {
