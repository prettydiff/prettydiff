...
var cleanCSS = function (x, size, character, comment, alter) {
    ...
    reduction = function () {
        ...
        ccex = new RegExp(/[\w\s:#\-\=\!\(\)"'\[\]\.%-\_\?\/\\]\/\*/),
        crex = new RegExp(/\/\*[\w\s:#\-\=\!\(\)"'\[\]\.%-\_\?\/\\]*;[\w\s:#\-\=\!\(\)"'\[\]\.%\_\?\/\\;]*\*\//),
        cceg = function (a) {
            return a.replace(/\s*\/\*/, ";/*");
        },
        creg = function (a) {
            if (crex.test(a) === false) {
                return a;
            }
            var b = a.length,
            c;
        ...
        for (b = a - 1; b > 0; b -= 1) {
            ...
            } else if (new RegExp(/[\}\s]/).test(x.charAt(b)) === false) {
                break;
            }
        }
        a = d.length - 1;
