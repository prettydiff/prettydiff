        ...
        reports     = function () {
            ...
            if (options.mode === "diff") {
                ...
                if (options.diffview !== "inline") {
                    ...
                    a.push("pd.difffold=function dom__difffold(self,min,max){\"use strict\";var a=0,b=0,inner=self.innerHTML,lists=[],parent=self.parentNode.parentNode,listnodes=(parent.getAttribute(\"class\"===\"diff\"))?parent.getElementsByTagName(\"ol\"):parent.parentNode.getElementsByTagName(\"ol\"),listLen=listnodes.length;for(a=0;a<listLen;a+=1){lists.push(listnodes[a].getElementsByTagName(\"li\"));}max=(max>=lists[0].length)?lists[0].length:max;if(inner.charAt(0)===\"-\"){self.innerHTML=\"+\"+inner.substr(1);for(a=min;a<max;a+=1){for(b=0;b<listLen;b+=1){lists[b][a].style.display=\"none\";}}}else{self.innerHTML=\"-\"+inner.substr(1);for(a=min;a<max;a+=1){for(b=0;b<listLen;b+=1){lists[b][a].style.display=\"block\";}}}};");
                    a.push("]]></script>");
                }
                a.push("</body></html>");
                return [
