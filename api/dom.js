...
(function dom__init() {
    ...
    pd.langkey             = function dom__langkey(all, obj, lang) {
        var value       = [],
            ...
            defaultt    = (pd.o.langdefault === null || pd.o.langdefault.nodeName.toLowerCase() !== "select") ? "javascript" : setlangmode(pd.o.langdefault[pd.o.langdefault.selectedIndex].value),
        ...
        if (pd.mode === "beau") {
            (function dom__recycle_beautify() {
                ...
                if (api.lang === "auto" || api.lang === "markup" || api.lang === "html" || api.lang === "xml" || api.lang === "jstl") {
                    ...
                    api.tagmerge     = (tagmerge === null || tagmerge.checked === false) ? false : true;
                }
            }());
            api.mode = "beautify";
        }
    ...
    pd.fixHeight           = function dom__fixHeight() {
        var baseText = pd.$$("baseText"),
            newText  = pd.$$("newText"),
            beauIn   = pd.$$("beautyinput"),
            beauOut  = pd.$$("beautyoutput"),
            minnIn   = pd.$$("minifyinput"),
            minnOut  = pd.$$("minifyoutput"),
            parsIn   = pd.$$("parseinput"),
            parsOut  = pd.$$("parseoutput"),
            math     = 0,
