...
(function dom__init() {
    ...
    pd.save                = function dom__save(x) {
        var top        = (x.parentNode.nodeName.toLowerCase() === "a")
            ...
            diffstring = "var pd={};pd.colSliderProperties=[];(function(){var d=document.getElementsByTagN" +
                        ...
                        "own=null;}};",
            beaustring = "pd.beaufold=function dom__beaufold(){var self=this,title=self.getAttribute('titl" +     "e').split('line '),min=Number(title[1].substr(0,title[1].indexOf(' '))),max=Numb"
                    ...
                    +     "}self.innerHTML='-'+self.innerHTML.substr(1);}};", span       = pd. id( "inline"), inline     = ( span !== null && span. checked !== false), type       = ""; if
        ( bodyInner. innerHTML === "") { return;
        }
        if
        ( inline === false) { type = document. getElementsByTagName( "script")[0].getAttribute( "type");
        }

        //added support for Firefox and Opera because they support long
        //URIs.  This extra support allows for local file creation.
        if
        ( x. nodeName. toLowerCase() === "a" && x. getElementsByTagName( "button")[0].innerHTML === "S") { if
            ( bodyInner === "" || ( (/Please\ try\ using\ the\ option\ labeled\ ((&lt;)|<)em((&gt;)|>)Plain\ Text\ \(diff\ only\)((&lt;)|<)\/em((&gt;)|>)\./). test( bodyInner) === true && ( /div\ class\=("|')diff("|')/). test( bodyInner) === false)) { return
                false;
            }
            build. push( "<?xml version='1.0' encoding='UTF-8' ?><!DOCTYPE html PUBLIC '-//W3C//DTD XHTML "+ "1.1//EN' 'http://www.w3.org/TR/xhtml11/DTD/xhtml11.dtd'><html xmlns='http://www."
                    ...
                    + "content='text/css'/><style type='text/css'>" + pd. css. core + pd. css["s" + pd. color] +"</style></head><body class='" + pd. color + "' id='webtool'><h1><a href='http://prettydiff.com/'>Pretty Diff - The difference" + " tool</a></h1><div id='doc'>"); if
            ( top === pd. o. report. code. box) { if
                ( pd. mode === "diff") { classQuote = ( bodyInner. indexOf( "<div class='diff'") > -1)
                        ? "<div class='diff'"
                        : "<div class=\"diff\"";
                } else
                if ( pd. mode === "beau") { classQuote = ( bodyInner. indexOf( "<div class='beautify'") > -1)
                        ? "<div class='beautify'"
                        : "<div class=\"beautify\"";
                }
                content = bodyInner. split( classQuote); build. push( content[0]); if
                ( content. length === 2) { build. push( "<p>Accessibility note. &lt;em&gt; tags in the output represent character differe"+ "nces per lines compared.</p></div>"); build. push( classQuote); build. push( content[1]); if
                    ( pd. mode === "diff") { build. push( "<script type='"); build. push( type); build. push( "'><![CDATA["); build. push( diffstring); build. push( "]]></script>");
                    } else
                    if ( pd. mode === "beau") { build. push( "<script type='"); build. push( type); build. push( "'><![CDATA["); build. push( beaustring); build. push( "]]></script>");
                    }
                }
            }
            build. push( "</body></html>"); x. setAttribute( "href", "data:text/prettydiff;charset=utf-8," + encodeURIComponent( build. join( ""))); x. onclick = function dom__save_rebind() { pd. save( this);
            };

            //prompt to save file created above.  below is the creation
            //of the modal with instructions about file extension.
            lastChild = pd. o. page. lastChild; if
            ( lastChild. nodeType > 1 || lastChild. nodeName. toLowerCase() === "script") { do
                { lastChild = lastChild. previousSibling;
                } while
                ( lastChild. nodeType > 1 || lastChild. nodeName. toLowerCase() === "script");
            }
            pageHeight = lastChild. offsetTop + lastChild. clientHeight + 20; lastChild  = document. createElement( "div"); lastChild. setAttribute( "onmousedown", "this.parentNode.removeChild(this)"); lastChild. setAttribute( "id", "modalSave"); span              = document. createElement( "span"); span. style. width  = ( pd. o. page. clientWidth + 10) + "px"; span. style. height = pageHeight + "px"; lastChild. appendChild( span); span           = document. createElement( "p"); span. innerHTML = "Just rename the file extension from '<strong>.part</strong>' to '<strong>.xhtml<" +              "/strong>'. <em>Click anywhere to close this reminder.</em>"; lastChild. appendChild( span); pd. o. page. appendChild( lastChild); span. style. left = ( ((pd. o. page. clientWidth + 10) - span. clientWidth) / 2) + "px"; return;
        }
        //Webkit and IE get the old functionality of a textarea with
        //HTML text content to copy and paste into a text file.
        pd. top( top); if
        ( (/Please\ try\ using\ the\ option\ labeled\ ((&lt;)|<)em((&gt;)|>)Plain\ Text\ \(diff\ only\)((&lt;)|<)\/em((&gt;)|>)\./). test( bodyInner) === true && ( /div\ class\=("|')diff("|')/). test( bodyInner) === false) { pd. o. report. code. body. innerHTML = "<p><strong>Error:</strong> Please try using the option labeled <em>Plain Text (d" + "iff only)</em>. <span style='display:block'>The input does not appear to be mark"; return;
        }
        if
        ( x. innerHTML === "S") { if
            ( pd. mode === "diff") { pd. o. save. checked = true;
            }
            if
            ( bodyInner !== "") { if
                ( top === pd. o. report. code. box) { if
                    ...
                    content    = bodyInner. split( classQuote); classQuote = classQuote + content[1];bodyInner  = content[0];build. push( " <p>This is the generated output. Please copy the text output, paste into a text"+ " file, and save as a &quot;.html&quot; file.</p> <textarea rows='40' cols='80' i"
                            + "d='textreport'>"); build. push( "&lt;?xml version='1.0' encoding='UTF-8' ?&gt;&lt;!DOCTYPE html PUBLIC '-//W3C//D"+ "TD XHTML 1.1//EN' 'http://www.w3.org/TR/xhtml11/DTD/xhtml11.dtd'&gt;&lt;html xml"
                            ...
                            + "'/&gt;&lt;style type='text/css'&gt;" + pd. css. core + pd. css["s" + pd. color] +"&lt;/style&gt;&lt;/head&gt;&lt;body class='" + pd. color + "' id='webtool'&gt;&lt;h1&gt;&lt;a href='http://prettydiff.com/'&gt;Pretty Diff -" + " The difference tool&lt;/a&gt;&lt;/h1&gt;&lt;div class=\"pdsavecontent\"&gt;"); build. push( bodyInner. replace( /&/g,"&amp;"). replace( />/g,"&gt;"). replace( /</g,"&lt;")); if
                    ( content. length === 2) { build. push( classQuote. replace( /&/g,"&amp;"). replace( />/g,"&gt;"). replace( /</g,"&lt;")); if
                        ( pd. mode === "diff") { build. push( "&lt;script type='"); build. push( type); build. push( "'&gt;&lt;![CDATA["); build. push( diffstring. replace( /&/g,"&amp;"). replace( />/g,"&gt;"). replace( /</g,"&lt;")); build. push( "]]&gt;&lt;/script&gt;");
                        } else
                        if ( pd. mode === "beau") { build. push( "&lt;script type='"); build. push( type); build. push( "'&gt;&lt;![CDATA["); build. push( beaustring. replace( /&/g,"&amp;"). replace( />/g,"&gt;"). replace( /</g,"&lt;")); build. push( "]]&gt;&lt;/script&gt;");
                        }
                    }
                }
                build. push( "&lt;/div&gt;&lt;/body&gt;&lt;/html&gt;</textarea>");
            }
            x. innerHTML = "H"; x. setAttribute( "title", "Convert output to rendered HTML."); body. innerHTML = build. join( "");
        } else
        { if
            ( pd. mode === "diff") { pd. o. save. checked = false;
            }
            if
            ( bodyInner !== "") { if
                ( bodyInner. indexOf( "<textarea") > -1) { bodyInner = bodyInner. replace( /&lt;/g,"<"). replace( /&gt;/g,">"). replace( /&amp;/g,"&");
                }
                classQuote = ( bodyInner. indexOf( "<div class='pdsavecontent'") > -1)
                    ? "<div class='pdsavecontent'>"
                    : "<div class=\"pdsavecontent\">"; content    = bodyInner. split( classQuote); if
                ( content[0].indexOf( "</h1>") > -1) { build. push( content[0].split( "</h1>")[1]);
                } else
                { build. push( content[0]);
                }
                if
                ( content. length > 1) { if
                    ( content[1].indexOf( "<script") > -1) { content[1] =content[1].substring( 0, content[1].indexOf( "<script"));
                    } else
                    if ( content[1].indexOf( "</body") > -1) { content[1] =content[1].substring( 0, content[1].indexOf( "</body") - 6);
                    }
                    build. push( content[1]);
                }
            }
            x. innerHTML = "S"; x. setAttribute( "title", "Convert report to text that can be saved."); body. innerHTML = build. join( ""); content        = body. getElementsByTagName( "ol"); if
            ( content. length > 0) { if
                ( pd. mode === "diff") { pd. colSliderProperties   = [
                    ...
                    ];content[2].onmousedown   = pd. colSliderGrab; content[2].ontouchstart = pd. colSliderGrab;
                }
                content = content[0].getElementsByTagName( "li"); for
                ( pageHeight = content. length - 1; pageHeight > -1; pageHeight -= 1) { if
                    ( content[pageHeight].getAttribute( "class") === "fold") { if
                        ( pd. mode === "beau") { content[pageHeight].onmousedown = pd. beaufold;
                        } else
                        if ( pd. mode === "diff") { content[pageHeight].onmousedown = pd. difffold;
                        }
                    }
                }
            }
    ...
    pd. grab                = function dom__grab( e, x) { var
        box        = x. parentNode. parentNode, parent     = box. getElementsByTagName( "p")[0],save       = ( parent. innerHTML. indexOf( "save") > -1), minifyTest = ( parent. style. display === "none"), buttons    = box. getElementsByTagName( "p")[0].getElementsByTagName( "button"), minButton  = ( save === true)
                ? buttons[1]
                :buttons[0],resize     = ( save === true)
                ? buttons[3]
                :buttons[2],body       = box. getElementsByTagName( "div")[0],heading    = ( box. firstChild. nodeType > 1)
                ? box. firstChild. nextSibling
                : box. firstChild, boxLeft    = box. offsetLeft, boxTop     = box. offsetTop, touchXNow  = 0, touchYNow  = 0, event      = e || window. event, touch      = ( e !== null && e. type === "touchstart"), filled     = ( (box === pd. o. report. stat. box && pd. test. filled. stat === true) || ( box === pd. o. report. feed. box && pd. test. filled. feed === true) || ( box === pd. o. report. code. box && pd. test. filled. code === true)), max        = document. getElementsByTagName( "body")[0].clientHeight, drop       = function dom__grab_drop() { var
                headingWidth = box. getElementsByTagName( "h3")[0].clientWidth; boxLeft = box. offsetLeft; boxTop  = box. offsetTop; if
                ( touch === true) { document. ontouchmove = null; document. ontouchend  = null;
                } else
                { document. onmousemove = null; document. onmouseup   = null;
                }
                if
                ( boxTop < 10) { box. style. top = "1em";
                } else
                if ( boxTop > ( max - 40)) { box. style. top = ( (max / 10) - 4) + "em";
                } else
                { box. style. top = ( boxTop / 10) + "em";
                }
                if
                ( boxLeft < ( (headingWidth * -1) + 40)) { box. style. left = ( ((headingWidth * -1) + 40) / 10) + "em";
                }
                body. style. opacity = "1"; box. style. height   = "auto"; heading. style. top  = "100%"; resize. style. top   = "100%"; pd. options( box); event. preventDefault(); return
                false;
            },
            boxmove    = function dom__grab_boxmove( f) { f = f || window. event; f. preventDefault(); box. style. right = "auto"; if
                ( touch === true) { box. style. left      = ( (boxLeft + ( f. touches[0].clientX - touchXNow)) / 10) + "em"; box. style. top       = ( (boxTop + ( f. touches[0].clientY - touchYNow)) / 10) + "em"; document. ontouchend = drop;
                } else
                { box. style. left     = ( (boxLeft + ( f. clientX - box. mouseX)) / 10) + "em"; box. style. top      = ( (boxTop + ( f. clientY - box. mouseY)) / 10) + "em"; document. onmouseup = drop;
                }
                return
                false;
            };
            e = e || window. event; if
        ( minifyTest === true) { if
            ( filled === true) { box. style. right = "auto";
            } else
            { box. style. left = "auto";
            }
            pd. minimize( e, 50, minButton); return
            false;
        }
        pd. top( box); if
        ...
        if
        ( heading. nodeType !== 1) { do
            { heading = heading. nextSibling;
            } while
            ( heading. nodeType !== 1);
        }
        heading = heading. lastChild; if
        ( heading. nodeType !== 1) { do
            { heading = heading. previousSibling;
            } while
            ( heading. nodeType !== 1);
        }
        body. style. opacity = ".5"; heading. style. top  = ( box. clientHeight / 20) + "0em"; box. style. height   = ".1em"; resize. style. top   = ( (Number( body. style. height. replace( "em", "")) + 5.27) / 1.9925) + "em"; box. mouseX         = e. clientX; box. mouseY         = e. clientY; if
        ( touch === true) { touchXNow             = e. touches[0].clientX; touchYNow             = e. touches[0].clientY; document. ontouchmove  = boxmove; document. ontouchstart = null;
        } else
        { document. onmousemove = boxmove; document. onmousedown = null;
        }
        pd. options( box); return
        false;
    };

    pd. feedsubmit          = function dom__feedsubmit( auto) { var
        datapack  = {},namecheck = ( localStorage. settings !== undefined)
                ? JSON. parse( localStorage. settings)
                : {},radios    = [],text      = ( pd. id( "feedtextarea") === null)
                ? ""
                : pd. id( "feedtextarea"). value, a         = 0, email     = ( pd. id( "feedemail") === null)
                ? ""
                : pd. id( "feedemail"). value, xhr       = {},sendit    = function dom__feedsubmit_sendit() { var
                node = pd. id( "feedintro"); xhr. withCredentials = true; xhr. open( "POST", "http://prettydiff.com:8000/feedback/", true); xhr. setRequestHeader( "Content-type", "application/json; charset=utf-8"); xhr. send( JSON. stringify( datapack)); pd. o. report. feed. box. getElementsByTagName( "button")[1].click(); if
                ( node !== null) { node. innerHTML = "Please feel free to submit feedback about Pretty Diff at any time by answering t" + "he following questions.";
                }
            };
            if
        ( pd. test. xhr === false || pd. test. json === false) { return;
        }
        xhr = ( typeofXMLHttpRequest === "function" || typeof XMLHttpRequest === "object")
            ? new XMLHttpRequest()
            : new ActiveXObject( "Microsoft.XMLHTTP"); if
        ( auto === true) { datapack = { name    : pd. settings. knownname, settings: pd. settings, stats   : pd. stat, type    : "auto"
            };
            sendit(); return;
        }
        if
        ( pd. id( "feedradio1") === null || namecheck. knownname !== pd. settings. knownname) { return;
        }
        radios = pd. id( "feedradio1"). parentNode. parentNode. getElementsByTagName( "input"); for
        ( a = radios. length - 1; a > -1; a -= 1) { if
            ( radios[a].checked === true) { break;
            }
        }
        if
        ( a < 0) { return;
        }
        datapack = { comment : text, email   : email, name    : pd. settings. knownname, rating  : a + 1, settings: pd. settings, stats   : pd. stat, type    : "feedback"
        };
        sendit();
    };

    //toggle between tool modes
    pd. prettyvis           = function dom__prettyvis( x) { var
        a           = {},b           = 0, lang        = ( pd. o. lang === null)
                ? "javascript"
                : ( (pd. o. lang. nodeName === "select")
                    ? pd. o. lang[pd. o. lang. selectedIndex].value
                    : pd. o. lang. value), langOps     = [],node        = {},storage     = "", langtest    = ( pd. o. lang !== null && pd. o. lang. nodeName. toLowerCase() === "select"), optioncheck = function dom__prettyvis_optioncheck() { var
                c     = 0, langs = [];langs = pd. o. lang. getElementsByTagName( "option"); for
                ( c = langs. length - 1; c > -1; c -= 1) { if
                    ( langs[c].value === "text") { if
                        ( pd. o. lang. selectedIndex === c) { pd. o. lang. selectedIndex = 0;
                        }
        ...
        if
        ( x. nodeType === 1) { if
            ( x. nodeName. toLowerCase() === "input") { a = x;
            } else
            { a = x. getElementsByTagName( "input")[0];
            }
        } else
        if ( this. nodeName. toLowerCase() === "input") { a = this;
        } else
        { a = this. getElementsByTagName( "input")[0];
        }
        node = pd. id( "showOptionsCallOut"); if
        ( node !== null) { node. parentNode. removeChild( node);
        }
        if
        ( a === pd. o. modeBeau) { pd. mode = "beau"; if
            ( langtest === true) { optioncheck();
            }
            if
            ( pd. o. codeBeauIn !== null) { if
                ( pd. o. codeBeauIn. value === "" && pd. o. codeMinnIn !== null && pd. o. codeMinnIn. value !== "") { pd. o. codeBeauIn. value = pd. o. codeMinnIn. value;
                } else
                if ( pd. o. codeBeauIn. value === "" && pd. o. codeBeauOut !== null && pd. o. codeBeauOut. value !== "") { pd. o. codeBeauIn. value = pd. o. codeBeauOut. value;
                }
            }
            if
            ( pd. o. beau !== null) { pd. o. beau. style. display = "block";
            }
            if
            ( pd. o. minn !== null) { pd. o. minn. style. display = "none";
            }
            if
            ( pd. o. pars !== null) { pd. o. pars. style. display = "none";
            }
            if
            ( pd. o. diffBase !== null) { pd. o. diffBase. style. display = "none";
            }
            if
            ( pd. o. diffNew !== null) { pd. o. diffNew. style. display = "none";
            }
            if
            ( pd. o. diffOps !== null) { pd. o. diffOps. style. display = "none";
            }
            if
            ( pd. o. minnOps !== null) { pd. o. minnOps. style. display = "none";
            }
            if
            ( pd. o. parsOps !== null) { pd. o. parsOps. style. display = "none";
            }
            if
            ( lang === "csv" && pd. o. beauOps !== null) { pd. o. beauOps. style. display = "none";
            } else
            if ( pd. o. beauOps !== null) { pd. o. beauOps. style. display = "block";
            }
            if
            ( pd. test. render. beau === false) { if
                ( pd. o. codeBeauIn !== null) { if
                    ( pd. test. ls === true && localStorage. codeBeautify !== undefined) { storage = localStorage. codeBeautify; if
                        ( (/^(\s+)$/). test( storage) === true) { storage = "";
                        }
                        if
                        ( pd. test. ace === true) { pd. ace. beauIn. setValue( storage); pd. ace. beauIn. clearSelection(); pd. langkey( false, pd. ace. beauIn, "");
                        } else
                        { pd. o. codeBeauIn. value = storage;
                        }
                    } else
                    if ( pd. test. ace === true) { pd. ace. beauIn. setValue( " ");
                    }
                }
                if
                ( pd. test. ace === true && pd. o. codeBeauOut !== null) { pd. ace. beauOut. setValue( " ");
                }
            }
            if
            ( pd. test. load === false && pd. o. jsscope !== null && pd. o. jsscope. checked === true) { pd. hideBeauOut( pd. o. jsscope);
            }
            pd. test. render. beau = true;
        }
        if
        ( a === pd. o. modeMinn) { pd. mode = "minn"; if
            ( langtest === true) { optioncheck();
            }
            if
            ( pd. o. codeMinnIn !== null) { if
                ( pd. o. codeMinnIn. value === "" && pd. o. codeBeauIn !== null && pd. o. codeBeauIn. value !== "") { pd. o. codeMinnIn. value = pd. o. codeBeauIn. value;
                } else
                if ( pd. o. codeMinnIn. value === "" && pd. o. codeBeauOut !== null && pd. o. codeBeauOut. value !== "") { pd. o. codeMinnIn. value = pd. o. codeBeauOut. value;
                }
            }
            if
            ( pd. o. minnOps !== null) { if
                ( lang === "text" || lang === "csv") { pd. o. minnOps. style. display = "none";
                } else
                { pd. o. minnOps. style. display = "block";
                }
            }
            if
            ( pd. o. minn !== null) { pd. o. minn. style. display = "block";
            }
            if
            ( pd. o. beau !== null) { pd. o. beau. style. display = "none";
            }
            if
            ( pd. o. pars !== null) { pd. o. pars. style. display = "none";
            }
            ...
            if
            ( pd. o. beauOps !== null) { pd. o. beauOps. style. display = "none";
            }
            if
            ( pd. o. parsOps !== null) { pd. o. parsOps. style. display = "none";
            }
            if
            ( pd. test. render. minn === false) { if
                ( pd. o. codeMinnIn !== null) { if
                    ( pd. test. ls === true && localStorage. codeMinify !== undefined) { storage = localStorage. codeMinify; if
                        ( (/^(\s+)$/). test( storage) === true) { storage = "";
                        }
                        if
                        ( pd. test. ace === true) { pd. ace. minnIn. setValue( storage); pd. ace. minnIn. clearSelection(); pd. langkey( false, pd. ace. minnIn, "");
                        } else
                        { pd. o. codeMinnIn. value = storage;
                        }
                    } else
                    if ( pd. test. ace === true) { pd. ace. minnIn. setValue( " ");
                    }
                }
                if
                ( pd. test. ace === true && pd. o. codeMinnOut !== null) { pd. ace. minnOut. setValue( " ");
                }
            }
            pd. test. render. minn = true;
        }
        if
        ( a === pd. o. modeDiff) { pd. mode = "diff"; if
            ( langtest === true) { langOps = pd. o. lang. getElementsByTagName( "option"); for
                ( b = langOps. length - 1; b > -1; b -= 1) { langOps[b].disabled = false;
                }
            }
            if
            ( pd. o. codeBeauOut !== null) { if
                ( pd. o. codeBeauOut. value === "" && pd. o. codeBeauIn !== null && pd. o. codeBeauIn. value !== "") { pd. o. codeBeauOut. value = pd. o. codeBeauIn. value;
                } else
                if ( pd. o. codeBeauOut. value === "" && pd. o. codeMinnIn !== null && pd. o. codeMinnIn. value !== "") { pd. o. codeBeauOut. value = pd. o. codeMinnIn. value;
                }
            }
            if
            ( pd. o. diffBase !== null) { pd. o. diffBase. style. display = "block";
            }
            if
            ( pd. o. diffNew !== null) { pd. o. diffNew. style. display = "block";
            }
            if
            ( pd. o. beau !== null) { pd. o. beau. style. display = "none";
            }
            ...
            if
            ( pd. o. diffOps !== null) { pd. o. diffOps. style. display = "block";
            }
            if
            ( pd. o. beauOps !== null) { pd. o. beauOps. style. display = "none";
            }
            ...
            if
            ( lang === "csv" || lang === "text") { node = pd. id( "diffquanp"); if
                ( node !== null) { node. style. display = "none";
                }
                node = pd. id( "difftypep"); if
                ( node !== null) { node. style. display = "none";
                }
                node = pd. id( "diffbeautify"); if
                ( node !== null) { node. style. display = "none";
                }
            } else
            { node = pd. id( "diffquanp"); if
                ( node !== null) { node. style. display = "block";
                }
                node = pd. id( "difftypep"); if
                ( node !== null) { node. style. display = "block";
                }
            ...
            if
            ( pd. test. render. diff === false) { if
                ( pd. o. codeDiffBase !== null) { if
                    ( pd. test. ls === true && localStorage. codeDiffBase !== undefined) { storage = localStorage. codeDiffBase; if
                        ( (/^(\s+)$/). test( storage) === true) { storage = "";
                        }
                        if
                        ( pd. test. ace === true) { pd. ace. diffBase. setValue( storage); pd. ace. diffBase. clearSelection(); if
                            ( lang !== "text") { pd. langkey( false, pd. ace. diffBase, "");
                            }
                        } else
                        { pd. o. codeDiffBase. value = storage;
                        }
                    } else
                    if ( pd. test. ace === true) { pd. ace. diffBase. setValue( " ");
                    }
                }
                if
                ( pd. o. codeDiffNew !== null) { if
                    ( pd. test. ls === true && localStorage. codeDiffNew !== undefined) { storage = localStorage. codeDiffNew; if
                        ( (/^(\s+)$/). test( storage) === true) { storage = "";
                        }
                        if
                        ( pd. test. ace === true) { pd. ace. diffNew. setValue( storage); pd. ace. diffNew. clearSelection(); if
                            ( lang !== "text") { pd. langkey( false, pd. ace. diffNew, "");
                            }
                        } else
                        { pd. o. codeDiffNew. value = storage;
                        }
                    } else
                    if ( pd. test. ace === true) { pd. ace. diffNew. setValue( " ");
                    }
                }
        ...
        if
        ( a === pd. o. modePars) { pd. mode = "pars"; if
            ( langtest === true) { optioncheck();
            }
            if
            ( pd. o. codeParsIn !== null) { if
                ( pd. o. codeParsIn. value === "" && pd. o. codeBeauIn !== null && pd. o. codeBeauIn. value !== "") { pd. o. codeParsIn. value = pd. o. codeBeauIn. value;
                } else
                if ( pd. o. codeParsIn. value === "" && pd. o. codeBeauOut !== null && pd. o. codeBeauOut. value !== "") { pd. o. codeParsIn. value = pd. o. codeBeauOut. value;
                }
            }
            if
            ( pd. o. parsOps !== null) { if
                ( lang === "text" || lang === "csv") { pd. o. parsOps. style. display = "none";
                } else
            ...
            if
            ( pd. o. pars !== null) { pd. o. pars. style. display = "block";
            }
            if
            ( pd. o. diffBase !== null) { pd. o. diffBase. style. display = "none";
            }
            ...
            if
            ( pd. test. render. pars === false) { if
                ( pd. o. codeParsIn !== null) { if
                    ( pd. test. ls === true && localStorage. codeParse !== undefined) { storage = localStorage. codeParse; if
                        ( (/^(\s+)$/). test( storage) === true) { storage = "";
                        }
                        if
                        ( pd. test. ace === true) { pd. ace. parsIn. setValue( storage); pd. ace. parsIn. clearSelection(); pd. langkey( false, pd. ace. parsIn, "");
                        } else
                        { pd. o. codeParsIn. value = storage;
                        }
                    } else
                    if ( pd. test. ace === true) { pd. ace. parsIn. setValue( " ");
                    }
                }
                if
                ( pd. test. ace === true && pd. o. codeParsOut !== null) { pd. ace. parsOut. setValue( " ");
                }
            }
            pd. test. render. pars = true;
        }
        if
        ( pd. o. announce !== null && ( a === pd. o. modeBeau || a === pd. o. modeMinn || a === pd. o. modeDiff)) { pd. o. announce. innerHTML = "";
        }
        if
        ( a. nodeType === undefined) { return;
        }
        pd. options( a);
    };

    pd. hideBeauOut         = function dom__hideBeauOut( x) { var
        node    = {},state   = false, restore = function dom__hideBeauOut_restore() { if
                ( pd. test. ace === true) { pd. o. codeBeauOut. parentNode. parentNode. style. display = "block";
                } else
                { pd. o. codeBeauOut. parentNode. style. display = "block";
                }
                if
                ( pd. o. codeBeauIn !== null) { if
                    ( pd. test. ace === true) { node             = pd. o. codeBeauIn. parentNode. parentNode; node. style. width = "49%"; pd. ace. beauIn. resize();
                    } else
                    { node             = pd. o. codeBeauIn. parentNode; node. style. width = "49%";
                    }
                    pd. o. codeBeauIn. onkeyup   = pd. recycle; pd. o. codeBeauIn. onkeydown = function dom_hideBeauOut_bindBeauInDown( e) { var
                        event = e || window. event; if
                        ( pd. test. ave === false) { pd. fixtabs( event, pd. o. codeBeauIn);
                        }
                        pd. keydown( event);
                    };
                }
            ...
            if
        ( x. nodeType === 1 && x. nodeName. toLowerCase() !== "input") { x = x. getElementsByTagName( "input")[0];
        }
        state = ( x === pd. o. jsscope); if
        ( pd. o. codeBeauOut === null) { return;
        }
        if
        ( pd. o. lang === null || pd. o. lang. value === "auto" || pd. o. lang. value === "javascript") { if
            ( state === true) { if
                ( pd. test. ace === true) { pd. o. codeBeauOut. parentNode. parentNode. style. display = "none";
                } else
                { pd. o. codeBeauOut. parentNode. style. display = "none";
                }
                if
                ( pd. o. codeBeauIn !== null) { if
                    ( pd. test. ace === true) { node             = pd. o. codeBeauIn. parentNode. parentNode; node. style. width = "100%"; pd. ace. beauIn. resize();
                    } else
                    { node             = pd. o. codeBeauIn. parentNode; node. style. width = "100%";
                    }
                    if
                    ( pd. test. ace === true) { pd. o. codeBeauIn. onkeyup = function dom__hideBeauOut_langkeyEditor( e) { var
                            event = e || window. event; pd. langkey( false, pd. ace. beauIn, ""); pd. recycle( event);
                        };
                    } else
                    { pd. o. codeBeauIn. onkeyup = function dom__hideBeauOut_langkeyTextarea( e) { var
                            event = e || window. event; pd. recycle( event);
                        };
                    }
                }
                pd. options( x);
    ...
    pd. codeOps             = function dom__codeOps( node) { var
        x    = {},lang = "", xml  = false, dqp  = pd. id( "diffquanp"), dqt  = pd. id( "difftypep"), db   = pd. id( "diffbeautify"), csvp = pd. id( "csvcharp"), hd   = pd. id( "htmld-yes"), he   = pd. id( "htmld-no"), hm   = pd. id( "htmlm-yes"), hn   = pd. id( "htmlm-no"), hp   = pd. id( "phtml-yes"), hq   = pd. id( "phtml-no"), hy   = pd. id( "html-yes"), hz   = pd. id( "html-no"); if
        ( node. nodeType === 1) { if
            ( node. nodeName. toLowerCase() === "input" || node. nodeName. toLowerCase() === "select") { x = node;
            } else
            { x = node. getElementsByTagName( "input")[0];
            }
        } else
        if ( this. nodeName. toLowerCase() === "input" || this. nodeName. toLowerCase() === "select") { x = this;
        } else
        { x = this. getElementsByTagName( "input")[0];
        }
        xml  = ( x. getElementsByTagName( "option")[x. selectedIndex].innerHTML === "XML" || x. getElementsByTagName( "option")[x. selectedIndex].innerHTML === "JSTL"); lang = ( pd. o. lang === null)
            ? "javascript"
            : ( pd. o. lang. nodeName === "select")
                ? pd. o. lang[pd. o. lang. selectedIndex].value
                : pd. o. lang. value; if
        ( pd. o. langvalue[0] ===undefined) { pd. o. langvalue[0] =lang;
        }
        if
        ( pd. o. modeDiff !== null && pd. o. modeDiff. checked === true) { if
            ( pd. o. minnOps !== null) { pd. o. minnOps. style. display = "none";
            }
            if
            ( pd. o. beauOps !== null) { pd. o. beauOps. style. display = "none";
            ...
            if
            ( lang === "text" || lang === "csv") { if
                ( dqp !== null) { dqp. style. display = "none";
                }
                if
                ( dqt !== null) { dqt. style. display = "none";
                }
            } else
            { if
                ( dqp !== null) { dqp. style. display = "block";
                }
                if
                ( dqt !== null) { dqt. style. display = "block";
                }
            }
        } else
        if ( pd. o. modeBeau !== null && pd. o. modeBeau. checked === true) { if
            ( pd. o. minnOps !== null) { pd. o. minnOps. style. display = "none";
            }
            if
            ( pd. o. diffOps !== null) { pd. o. diffOps. style. display = "none";
            ...
            if
            ( pd. o. beauOps !== null) { if
                ( lang === "csv") { pd. o. beauOps. style. display = "none";
                } else
                { pd. o. beauOps. style. display = "block";
                }
            }
        } else
        if ( pd. o. modeMinn !== null && pd. o. modeMinn. checked === true) { if
            ( pd. o. beauOps !== null) { pd. o. beauOps. style. display = "none";
            }
            if
            ( pd. o. diffOps !== null) { pd. o. diffOps. style. display = "none";
            ...
            if
            ( pd. o. minnOps !== null) { if
                ( lang === "csv") { pd. o. minnOps. style. display = "none";
                } else
                { pd. o. minnOps. style. display = "block";
                }
            }
        } else
        if ( pd. o. modePars !== null && pd. o. modePars. checked === true) { if
            ( pd. o. beauOps !== null) { pd. o. beauOps. style. display = "none";
            }
            if
            ( pd. o. diffOps !== null) { pd. o. diffOps. style. display = "none";
            ...
            if
            ( pd. o. minnOps !== null) { if
                ( lang === "csv") { pd. o. parsOps. style. display = "none";
                } else
                { pd. o. parsOps. style. display = "block";
                }
            }
        ...
        if
        ( csvp !== null) { if
            ( lang === "csv") { csvp. style. display = "block";
            } else
        ...
        if
        ( db !== null) { if
            ( lang === "csv" || lang === "text") { db. style. display = "none";
            } else
        ...
        if
        ( lang === "html") { if
            ( hd !== null) { hd. checked = true;
            }
            if
            ( hm !== null) { hm. checked = true;
            }
            if
            ( hy !== null) { hy. checked = true;
            }
            if
            ( hp !== null) { hp. checked = true;
            }
        } else
        if ( xml === true) { if
            ( he !== null) { he. checked = true;
            }
            if
            ( hn !== null) { hn. checked = true;
            }
            if
            ( hz !== null) { hz. checked = true;
            }
            if
            ( hq !== null) { hq. checked = true;
            }
        } else
        { if
            ( pd. settings. presumehtmld === "htmld-no" && he !== null) { he. checked = true;
            }
            if
            ( pd. settings. presumehtmlm === "htmlm-no" && hn !== null) { hn. checked = true;
            }
            if
            ( pd. settings. presumehtml === "html-no" && hz !== null) { hz. checked = true;
            }
            if
            ( pd. settings. presumehtml === "phtml-no" && hq !== null) { hq. checked = true;
            }
            if
            ( pd. settings. presumehtmld === "htmld-yes" && hd !== null) { hd. checked = true;
            }
            if
            ( pd. settings. presumehtmlm === "htmlm-yes" && hm !== null) { hm. checked = true;
            }
            if
            ( pd. settings. presumehtml === "html-yes" && hy !== null) { hy. checked = true;
            }
            if
            ( pd. settings. presumehtml === "phtml-yes" && hp !== null) { hp. checked = true;
            }
        }
        if
        ( x === pd. o. lang) { if
            ( pd. o. langdefault !== null) { if
                ( lang === "auto") { pd. o. langdefault. parentNode. style. display = "block"; pd. o. langdefault. disabled                 = false;
                } else
                { pd. o. langdefault. parentNode. style. display = "none";
                }
            }
            if
            ( lang === "auto") { pd. langkey( true, {},"");
            } else
            { pd. langkey( true, {},lang);
            }
            if
            ( pd. test. load === false) { pd. hideBeauOut( x);
            }
        }
        pd. options( x);
    };
    ...
    pd. indentchar          = function dom__indentchar( x) { var
        node      = {},quan      = {},beauChar  = pd. id( "beau-char"), diffChar  = pd. id( "diff-char"), beauOther = pd. id( "beau-other"), diffOther = pd. id( "diff-other"); if
        ( x. nodeType === 1) { if
            ( x. nodeName. toLowerCase() === "input") { node = x;
            } else
            { node = x. getElementsByTagName( "input")[0];
            }
        } else
        if ( this. nodeName. toLowerCase() === "input") { node = this;
        } else
        { node = this. getElementsByTagName( "input")[0];
        }
        if
        ( pd. mode === "beau" && beauOther !== null && beauChar !== null) { if
            ( node === beauOther || node === beauChar) { beauOther. checked = true; beauChar. setAttribute( "class", "checked"); if
                ( beauChar. value === "Click me for custom input") { beauChar. value = "";
                }
            } else
            { beauOther. checked = false; beauChar. setAttribute( "class", "unchecked"); if
                ( beauChar. value === "") { beauChar. value = "Click me for custom input";
                }
            }
        } else
        if ( pd. mode === "diff" && diffOther !== null && diffChar !== null) { if
            ( node === diffOther || node === diffChar) { diffOther. checked = true; diffChar. setAttribute( "class", "checked"); if
                ( diffChar. value === "Click me for custom input") { diffChar. value = "";
                }
            } else
            { diffOther. checked = false; diffChar. setAttribute( "class", "unchecked"); if
                ( diffChar. value === "") { diffChar. value = "Click me for custom input";
                }
            }
        }
        if
        ( pd. test. ace === true) { if
            ( pd. mode === "diff") { if
                ( node === pd. id( "diff-space")) { pd. ace. diffBase. getSession(). setUseSoftTabs( true); pd. ace. diffNew. getSession(). setUseSoftTabs( true); pd. ace. minnIn. getSession(). setUseSoftTabs( true); pd. ace. minnOut. getSession(). setUseSoftTabs( true); pd. ace. parsIn. getSession(). setUseSoftTabs( true); pd. ace. parsOut. getSession(). setUseSoftTabs( true); quan = pd. id( "diff-quan"); if
                    ( quan !== null && isNaN( Number( quan. value)) === false) { pd. ace. diffBase. getSession(). setTabSize( Number( node. value)); pd. ace. diffNew. getSession(). setTabSize( Number( node. value)); pd. ace. minnIn. getSession(). setTabSize( Number( node. value)); pd. ace. minnOut. getSession(). setTabSize( Number( node. value)); pd. ace. parsIn. getSession(). setTabSize( Number( node. value)); pd. ace. parsOut. getSession(). setTabSize( Number( node. value));
                    }
                } else
                { pd. ace. diffBase. getSession(). setUseSoftTabs( false); pd. ace. diffNew. getSession(). setUseSoftTabs( false); pd. ace. minnIn. getSession(). setUseSoftTabs( false); pd. ace. minnOut. getSession(). setUseSoftTabs( false); pd. ace. parsIn. getSession(). setUseSoftTabs( false); pd. ace. parsOut. getSession(). setUseSoftTabs( false);
                }
            }
            if
            ( pd. mode === "beau") { if
                ( node === pd. id( "beau-space")) { pd. ace. beauIn. getSession(). setUseSoftTabs( true); pd. ace. beauOut. getSession(). setUseSoftTabs( true); pd. ace. minnIn. getSession(). setUseSoftTabs( true); pd. ace. minnOut. getSession(). setUseSoftTabs( true); pd. ace. parsIn. getSession(). setUseSoftTabs( true); pd. ace. parsOut. getSession(). setUseSoftTabs( true); quan = pd. id( "beau-quan"); if
                    ( quan !== null && isNaN( Number( quan. value)) === false) { pd. ace. beauIn. getSession(). setTabSize( Number( node. value)); pd. ace. beauOut. getSession(). setTabSize( Number( node. value)); pd. ace. minnIn. getSession(). setTabSize( Number( node. value)); pd. ace. minnOut. getSession(). setTabSize( Number( node. value)); pd. ace. parsIn. getSession(). setTabSize( Number( node. value)); pd. ace. parsOut. getSession(). setTabSize( Number( node. value));
                    }
                } else
                { pd. ace. beauIn. getSession(). setUseSoftTabs( false); pd. ace. beauOut. getSession(). setUseSoftTabs( false); pd. ace. minnIn. getSession(). setUseSoftTabs( false); pd. ace. minnOut. getSession(). setUseSoftTabs( false); pd. ace. parsIn. getSession(). setUseSoftTabs( false); pd. ace. parsOut. getSession(). setUseSoftTabs( false);
                }
            }
        }
        if
        ( node === beauChar) { pd. options( beauChar);
        } else
        if ( node === diffChar) { pd. options( diffChar);
        } else
        { pd. options( node);
        }
    };
    ...
    pd. options             = function dom__options( x) { var
        item   = {},node   = "", name   = "", type   = "", id     = "", classy = "", h3     = {},body   = {};if
        ( x !== undefined && x. nodeType === 1) { if
            ( x. nodeName. toLowerCase() === "input" || x. nodeName. toLowerCase() === "select" || x. nodeName. toLowerCase() === "div") { item = x;
            } else
            { item = x. getElementsByTagName( "input")[0];if
                ( item === undefined) { return;
                }
            }
        } else
        if ( this. nodeName. toLowerCase() === "input" || this. nodeName. toLowerCase() === "select" || this. nodeName. toLowerCase() === "div") { item = this;
        } else
        ...
        if
        ( item. nodeName. toLowerCase() !== "div" && pd. test. load === false) { item. focus();
        }
        node   = item. nodeName. toLowerCase(); name   = item. getAttribute( "name"); type   = item. getAttribute( "type"); id     = item. getAttribute( "id"); classy = item. getAttribute( "class"); if
        ( pd. test. load === true) { return;
        }
        if
        ( node === "input") { if
            ( type === "radio") { if
                ( id === "beau-other") { pd. settings. beauchar = pd. id( "beau-char"). value;
                } else
                { pd. settings[name] =id;
                }
                if
                ( id === "diff-other") { pd. settings. diffchar = pd. id( "diff-char"). value;
                } else
                { pd. settings[name] =id;
                }
            } else
            if ( type === "text") { if
                ( id === "beau-char") { pd. settings. beauchar = item. value;
                } else
                if ( id === "diff-char") { pd. settings. diffchar = item. value;
                } else
                { pd. settings[id] =item. value;
                }
            }
        } else
        if ( node === "select") { pd. settings[id] =item. selectedIndex;
        } else
        if ( node === "div" && classy === "box") { h3   = item. getElementsByTagName( "h3")[0];body = item. getElementsByTagName( "div")[0];if
            ( pd. settings[id] ===undefined) { pd. settings[id] ={};
            }
            if
            ( body. style. display === "none" && h3. clientWidth < 175) { pd. settings[id].min  = true; pd. settings[id].max = false;
            } else
            if ( pd. settings[id].max === false || pd. settings[id].max === undefined) { pd. settings[id].min   = false; pd. settings[id].left = item. offsetLeft; pd. settings[id].top  = item. offsetTop; if
                ( pd. test. agent. indexOf( "macintosh") > 0) { pd. settings[id].width   = ( body. clientWidth - 20); pd. settings[id].height = ( body. clientHeight - 53);
                } else
                { pd. settings[id].width   = ( body. clientWidth - 4); pd. settings[id].height = ( body. clientHeight - 36);
                }
            }
        } else
        if ( node === "button" && id !== null) { pd. settings[id] =item. innerHTML. replace( /\s+/g," ");
        }
        if
        ( pd. test. json === true && pd. test. ls === true) { localStorage. settings = JSON. stringify( pd. settings);
        }

        //pd.comment additions
        if
        ( pd. o. comment !== null && id !== null) { (
            functiondom__options_comment() { var
                a    = 0, data = [];if
                ( id === "baselabel") { data = [
                ...
                if
                ( id === "bbraceline-no" || id === "dbraceline-no") { data = [
                        "braceline", "false"
                    ];
                }
                if
                ( id === "bbraceline-yes" || id === "dbraceline-yes") { data = [
                        "braceline", "true"
                    ];
                }
                if
                ( id === "bjslines-all" || id === "djslines-all") { data = [
                        "preserve", "all"
                    ];
                }
                if
                ( id === "bjslines-css" || id === "djslines-css") { data = [
                        "preserve", "css"
                    ];
                }
                if
                ( id === "bjslines-js" || id === "djslines-js") { data = [
                        "preserve", "js"
                    ];
                }
                if
                ( id === "bjslines-none" || id === "djslines-none") { data = [
                        "preserve", "none"
                    ];
                }
                if
                ( id === "bobjsort-all" || id === "dobjsort-all" || id === "mobjsort-all") { data = [
                        "objsort", "all"
                    ];
                }
                if
                ( id === "bobjsort-css" || id === "dobjsort-css" || id === "mobjsort-css") { data = [
                        "objsort", "css"
                    ];
                }
                if
                ( id === "bobjsort-js" || id === "dobjsort-js" || id === "mobjsort-js") { data = [
                        "objsort", "js"
                    ];
                }
                if
                ( id === "bobjsort-none" || id === "dobjsort-none" || id === "mobjsort-none") { data = [
                        "objsort", "none"
                    ];
                }
                if
                ( id === "bquoteconvert-double" || id === "mquoteconvert-double") { data = [
                        "quoteConvert", "double"
                    ];
                }
                if
                ( id === "bquoteconvert-none" || id === "mquoteconvert-none") { data = [
                        "quoteConvert", "none"
                    ];
                }
                if
                ( id === "bquoteconvert-single" || id === "mquoteconvert-single") { data = [
                        "quoteConvert", "single"
                    ];
                }
                if
                ( id === "bstyleguide") { if
                    ( item[item. selectedIndex].value === "") { data = [
                            "styleguide", "\"\""
                        ];
                    } else
                    { data = [
                ...
                if
                ( id === "btagmerge-no" || id === "dtagmerge-no" || id === "mtagmerge-no" || id === "ptagmerge-no") { data = [
                        "tagmerge", "false"
                    ];
                }
                if
                ( id === "btagmerge-yes" || id === "dtagmerge-yes" || id === "mtagmerge-yes" || id === "ptagmerge-yes") { data = [
                        "tagmerge", "true"
                    ];
                }
                if
                ( id === "bvarword-each") { data = [
                        "varword", "each"
                    ];
                }
                if
                ( id === "bvarword-list") { data = [
                        "varword", "list"
                    ];
                }
                if
                ( id === "bvarword-none") { data = [
                        "varword", "none"
                    ];
                }
                if
                ( id === "conditionald-no" || id === "conditionalm-no") { data = [
                        "conditional", "false"
                    ];
                }
                if
                ( id === "conditionald-yes" || id === "conditionalm-yes") { data = [
                        "conditional", "true"
                    ];
                }
                if
                ...
                if
                ( id === "diff-char" || id === "beau-char") { data = [
                        "inchar", "\"" + item. value + "\""
                    ];
                }
                if
                ( id === "diff-line" || id === "beau-line") { data = [
                        "inchar", "\"\\n\""
                    ];
                }
                if
                ( id === "diff-quan" || id === "beau-quan" || id === "minn-quan") { data = [
                        "insize", "\"" + item. value + "\""
                    ];
                }
                if
                ( id === "diff-space" || id === "beau-space") { data = [
                        "inchar", "\" \""
                    ];
                }
                if
                ( id === "diff-tab" || id === "beau-tab") { data = [
                        "inchar", "\"\\t\""
                    ];
                }
                if
                ( id === "diff-wrap" || id === "beau-wrap") { data = [
                        "wrap", "\"" + item. value + "\""
                    ];
                }
                if
                ...
                if
                ( id === "diffcontenty") { data = [
                        "content", "false"
                    ];
                }
                if
                ( id === "dforce_indent" || id === "bforce_indent") { data = [
                        "force_indent", "true"
                    ];
                }
                if
                ( id === "dforce_indent-no" || id === "bforce_indent-no") { data = [
                        "force_indent", "false"
                    ];
                }
                if
                ( id === "diffcommentsn") { data = [
                        "diffcomments", "false"
                    ];
                }
                if
                ( id === "diffcommentsy") { data = [
                        "diffcomments", "true"
                    ];
                }
                if
                ...
                if
                ( id === "htmld-no" || id === "html-no" || id === "htmlm-no") { data = [
                        "html", "false"
                    ];
                }
                if
                ( id === "htmld-yes" || id === "html-yes" || id === "htmlm-yes") { data = [
                        "html", "true"
                    ];
                }
                if
                ( id === "incomment-no") { data = [
                        "comments", "noindent"
                    ];
                }
                if
                ( id === "incomment-yes") { data = [
                        "comments", "indent"
                    ];
                }
                if
                ...
                if
                ( id === "inscriptd-no" || id === "inscript-no") { data = [
                        "style", "noindent"
                    ];
                }
                if
                ( id === "inscriptd-yes" || id === "inscript-yes") { data = [
                        "style", "indent"
                    ];
                }
                if
                ( id === "jscorrect-no" || id === "mjscorrect-no") { data = [
                        "correct", "false"
                    ];
                }
                if
                ( id === "jscorrect-yes" || id === "mjscorrect-yes") { data = [
                        "correct", "true"
                    ];
                }
                if
                ( id === "jselseline-no") { data = [
                        "elseline", "false"
                    ];
                }
                if
                ( id === "jselseline-yes") { data = [
                        "elseline", "true"
                    ];
                }
                if
                ( id === "jsindentd-all" || id === "jsindent-all") { data = [
                        "indent", "allman"
                    ];
                }
                if
                ( id === "jsindentd-knr" || id === "jsindent-knr") { data = [
                        "indent", "knr"
                    ];
                }
                if
                ( id === "jsscope-html") { data = [
                        "jsscope", "true"
                    ];
                }
                if
                ...
                if
                ( id === "jsspaced-no" || id === "jsspace-no") { data = [
                        "jsspace", "false"
                    ];
                }
                if
                ( id === "jsspaced-yes" || id === "jsspace-yes") { data = [
                        "jsspace", "true"
                    ];
                }
                if
                ( id === "lang-default") { data = [
                ...
                if
                ( id === "vertical-all") { data = [
                        "vertical", "all"
                    ];
                }
                if
                ( id === "vertical-cssonly") { data = [
                        "vertical", "css"
                    ];
                }
                if
                ( id === "vertical-jsonly") { data = [
                        "vertical", "js"
                    ];
                }
                if
                ( id === "vertical-none") { data = [
                        "vertical", "none"
                    ];
                }
                if
                ...
                for
                ( a = pd. commentString. length - 1; a > -1; a -= 1) { if
                    ( pd. commentString[a].indexOf( data[0]) > -1) { pd. commentString[a] =data. join( ": "); break;
                    }
                }
                if
                ( a < 0) { pd. commentString. push( data. join( ": ")); pd. commentString. sort();
                }
                if
                ( pd. o. comment !== null) { if
                    ( pd. commentString. length === 0) { pd. o. comment. innerHTML = "/*prettydiff.com */";
                    } else
                    if ( pd. commentString. length === 1) { pd. o. comment. innerHTML = "/*prettydiff.com " + pd. commentString[0].replace( /&/g,"&amp;"). replace( /</g,"&lt;"). replace( />/g,"&gt;"). replace( /api\./g,"") + " */";
                    } else
                    { pd. o. comment. innerHTML = "/*prettydiff.com " + pd. commentString. join( ", "). replace( /&/g,"&amp;"). replace( /</g,"&lt;"). replace( />/g,"&gt;"). replace( /api\./g,"") + " */";
                    }
                }
                if
                ( pd. test. ls === true && pd. test. json === true) { localStorage. commentString = JSON. stringify( pd. commentString). replace( /api\./g,"");
                }
            }());
        }
    };

    pd. clearComment        = function dom__clearComment() { localStorage. commentString = "[]"; pd. commentString           = [];if
        ( pd. o. comment !== null) { pd. o. comment. innerHTML = "/*prettydiff.com */";
        }
    };

    //reset tool to default configuration
    pd. reset               = function dom__reset() { var
        nametry = "", name    = ""; localStorage. codeBeautify  = ""; localStorage. codeDiffBase  = ""; localStorage. codeDiffNew   = ""; localStorage. codeMinify    = ""; localStorage. codeParse     = ""; localStorage. commentString = "[]"; if
        ( pd. settings === undefined || pd. settings. knownname === undefined) { if
            ( pd. settings === undefined) { pd. settings = { feedback: { newb   : false, veteran: false
                    }
                };
            }
            if
            ( localStorage. settings !== undefined) { nametry = JSON. stringify( localStorage. settings);
            }
            if
            ( localStorage. settings === undefined || nametry. knownname === undefined || nametry === "" || nametry. indexOf( "knownname") < 0) { name = "\"" + Math. random(). toString(). slice( 2) + Math. random(). toString(). slice( 2) + "\"";
            }
            pd. settings. knownname = name;
        }
        if
        ( pd. settings. feedback === undefined) { pd. settings. feedback = { newb   : false, veteran: false
            };
        }
        localStorage. settings = "{\"feedback\":{\"newb\":" + pd. settings. feedback. newb + ",\"veteran\":" + pd. settings. feedback. veteran + "},\"codereport\":{},\"statreport\":{},\"knownname\":" + pd. settings. knownname + "}"; pd. commentString      = [];if
        ( pd. o. comment !== null) { pd. o. comment. innerHTML = "/*prettydiff.com */";
        }
        pd. o. modeDiff. checked = true; location. reload();
    };

    pd. fixHeight           = function dom__fixHeight() { var
        baseText = pd. id( "baseText"), newText  = pd. id( "newText"), beauIn   = pd. id( "beautyinput"), beauOut  = pd. id( "beautyoutput"), minnIn   = pd. id( "minifyinput"), minnOut  = pd. id( "minifyoutput"), parsIn   = pd. id( "parseinput"), parsOut  = pd. id( "parseoutput"), math     = 0, height   = window. innerHeight || document. getElementsByTagName( "body")[0].clientHeight, headline = 0; if
        ( pd. o. headline !== null && pd. o. headline. style. display === "block") { headline = 3.8;
        }
        if
        ( pd. test. ace === true) { if
            ( baseText !== null && newText !== null) { math                  = ( height / 12) - ( 19.5 + headline); baseText. style. height = math + "em"; newText. style. height  = math + "em"; pd. ace. diffBase. setStyle( "height:" + math + "em"); pd. ace. diffNew. setStyle( "height:" + math + "em"); pd. ace. diffBase. resize(); pd. ace. diffNew. resize();
            }
            math = ( height / 12) - ( 18.12 + headline); if
            ( pd. o. codeBeauIn !== null) { beauIn. style. height = math + "em"; pd. ace. beauIn. setStyle( "height:" + math + "em"); pd. ace. beauIn. resize();
            }
            if
            ( pd. o. codeMinnIn !== null) { minnIn. style. height = math + "em"; pd. ace. minnIn. setStyle( "height:" + math + "em"); pd. ace. minnIn. resize();
            }
            if
            ( pd. o. codeParsIn !== null) { parsIn. style. height = math + "em"; pd. ace. parsIn. setStyle( "height:" + math + "em"); pd. ace. parsIn. resize();
            }
            math = ( height / 12) - ( 15.725 + headline); if
            ( pd. o. codeBeauOut !== null) { beauOut. style. height = math + "em"; pd. ace. beauOut. setStyle( "height:" + math + "em"); pd. ace. beauOut. resize();
            }
            if
            ( pd. o. codeMinnOut !== null) { minnOut. style. height = math + "em"; pd. ace. minnOut. setStyle( "height:" + math + "em"); pd. ace. minnOut. resize();
            }
            if
            ( pd. o. codeParsOut !== null) { parsOut. style. height = math + "em"; pd. ace. parsOut. setStyle( "height:" + math + "em"); pd. ace. parsOut. resize();
            }
        } else
        { if
            ( baseText !== null && newText !== null) { math                  = ( height / 12) - ( 20 + headline); baseText. style. height = math + "em"; newText. style. height  = math + "em";
            }
            math = ( height / 12) - ( 18.65 + headline); if
            ( pd. o. codeBeauIn !== null) { pd. o. codeBeauIn. style. height = math + "em";
            }
            if
            ( pd. o. codeMinnIn !== null) { pd. o. codeMinnIn. style. height = math + "em";
            }
            if
            ( pd. o. codeParsIn !== null) { pd. o. codeParsIn. style. height = math + "em";
            }
            math = ( height / 12) - ( 16.3 + headline); if
            ( pd. o. codeBeauOut !== null) { pd. o. codeBeauOut. style. height = math + "em";
            }
            if
            ( pd. o. codeMinnOut !== null) { pd. o. codeMinnOut. style. height = math + "em";
            }
            if
            ( pd. o. codeParsOut !== null) { pd. o. codeParsOut. style. height = math + "em";
            }
        }
    };
    ...
    (
    functiondom__load() { var
        a             = 0, inputs        = [],inputsLen     = 0, id            = "", name          = "", type          = "", node          = {},buttons       = {},title         = {},statdump      = [],thirdparty    = function dom__load_thirdparty() { var
                that = this, href = that. getAttribute( "href"); window. open( href, "thirdparty"); return
                false;
            },
            resize        = function dom__load_resize( e) { var
                that = this; pd. resize( e, that);
            },
            save          = function dom__load_save() { var
                that = this; pd. save( that);
            },
            grab          = function dom__load_grab( e) { var
                that = this; pd. grab( e, that);
            },
            top           = function dom__load_top() { var
                that = this; pd. top( that. parentNode);
            },
            page          = ( pd. o. page === null)
                ? ""
                : pd. o. page. getAttribute( "id"), backspace     = function dom__load_backspace( event) { var
                aa = event || window. event, bb = aa. srcElement || aa. target; if
                ( aa. keyCode === 8) { if
                    ( bb. nodeName === "textarea" || ( bb. nodeName === "input" && ( bb. getAttribute( "type") === "text" || bb. getAttribute( "type") === "password"))) { return
                        true;
                    }
                    return
                    false;
                ...
                if
                ( aa. type === "keydown") { pd. sequence( aa);
                }
            },
            acedisable    = function dom__load_acedisable( x) { var
                el     = ( typeofx === "object" && x. nodeType === 1)
                        ? x
                        : this, elId   = el. getAttribute( "id"), loc    = location. href. indexOf( "ace=false"), place  = [],symbol = "?"; pd. options( el); if
                ( elId. indexOf( "-yes") > 0 && loc > 0) { place = location. href. split( "ace=false"); if
                    ( place[1].indexOf( "&") < 0 && place[1].indexOf( "%26") < 0) { place[0]      =place[0].slice( 0, place[0].length - 1); location. href = place. join( "");
                    }
                } else
                if ( elId. indexOf( "-no") > 0 && loc < 0) { if
                    ( location. href. indexOf( "?") < location. href. length - 1 && location. href. indexOf( "?") > 0) { symbol = "&";
                    }
                    location. href = location. href + symbol + "ace=false";
                }
            },
            hideBeauOut   = function dom__load_hideBeauOut() { pd. hideBeauOut( this);
            },
            feedradio     = function dom__load_feedradio( e) { var
                event  = e || window. event, item   = this. parentNode, radio  = item. getElementsByTagName( "input")[0],radios = item. parentNode. getElementsByTagName( "input"), aa     = 0; for
                ( aa = radios. length - 1; aa > -1; aa -= 1) { radios[aa].parentNode. removeAttribute( "class"); radios[aa].checked = false;
                }
                radio. checked = true; radio. focus(); item. setAttribute( "class", "active-focus"); event. preventDefault(); return
                false;
            },
            feedblur      = function dom__load_feedblur() { var
                label = this. parentNode; label. setAttribute( "class", "active");
            },
            textareafocus = function dom__load_textareafocus() { var
                tabkey = pd. id( "textareaTabKey"), aria   = pd. id( "arialive"); if
                ( tabkey === null) { return;
                }
                tabkey. style. zIndex = pd. zIndex + 10; if
                ( aria !== null) { aria. innerHTML = tabkey. innerHTML;
                }
                if
                ( pd. mode === "diff") { tabkey. style. right = "51%"; tabkey. style. left  = "auto";
                } else
                { tabkey. style. left  = "51%"; tabkey. style. right = "auto";
                }
                tabkey. style. display = "block"; if
                ( pd. test. ace === true) { this. parentNode. parentNode. setAttribute( "class", this. parentNode. parentNode. getAttribute( "class") + " filefocus");
                }
            },
            textareablur  = function dom__load_textareablur() { var
                tabkey = pd. id( "textareaTabKey"); if
                ( tabkey === null) { return;
                }
                tabkey. style. display = "none"; if
                ( pd. test. ace === true) { this. parentNode. parentNode. setAttribute( "class", this. parentNode. parentNode. getAttribute( "class"). replace( " filefocus", ""));
                }
            },
            filefocus     = function dom__load_filefocus() { this. setAttribute( "class", "filefocus");
            },
            fileblur      = function dom__load_fileblur() { this. removeAttribute( "class");
            },
            savecheck     = function dom__load_savecheck() { var
                button = {};if
                ( pd. o. report. code. box === null) { return;
                }
                button = pd. o. report. code. box. getElementsByTagName( "button")[0];if
                ( button. getAttribute( "class") !== "save") { return;
                }
                if
                ( this. checked === true) { button. innerHTML = "H";
                } else
                { button. innerHTML = "S";
                }
            },
            headerfocus   = function dom__load_headerfocus() { this. onclick = pd. minimize;
            },
            headerblur    = function dom__load_headerblur() { this. onclick = null;
            };
            pd. fixHeight(); window. onresize = pd. fixHeight; window. onkeyup  = pd. areaShiftUp; if
        ( page === "webtool") { if
            ( pd. o. announce !== null) { pd. o. announcetext = pd. o. announce. innerHTML;
            }
            node = pd. id( "hideOptions"); if
            ( pd. id( "option_commentClear") !== null) { pd. id( "option_commentClear"). onclick = pd. clearComment;
            }
            document. onkeypress          = backspace; document. onkeydown           = backspace; pd. zIndex                    = 10; pd. mode                      = "diff"; pd. settings                  = {};pd. settings. feedreport       = {};pd. settings. codereport       = {};pd. settings. statreport       = {};pd. settings. feedback         = {};pd. settings. feedback. newb    = false; pd. settings. feedback. veteran = false; pd. settings. knownname        = "\"" + Math. random(). toString(). slice( 2) + Math. random(). toString(). slice( 2) + "\""; pd. keypress                  = { date    : {},keys    : [],state   : false, throttle: 0
            };
            if
            ( pd. test. fs === false) { node = pd. id( "diffbasefile"); if
                ( node !== null) { node. disabled = true;
                }
                node = pd. id( "diffnewfile"); if
                ( node !== null) { node. disabled = true;
                }
                node = pd. id( "beautyfile"); if
                ( node !== null) { node. disabled = true;
                }
                node = pd. id( "minifyfile"); if
                ( node !== null) { node. disabled = true;
                }
            }
            if
            ( pd. test. ls === true) { if
                ( localStorage. webtool !== undefined) { delete
                    localStorage. webtool; delete
                    localStorage. optionString;
                }
                if
                ( localStorage. bl !== undefined) { delete
                    localStorage. bl;
                }
                if
                ( localStorage. nl !== undefined) { delete
                    localStorage. nl;
                }
                if
                ( localStorage. bo !== undefined) { name = localStorage. bo; delete
                    localStorage. bo; localStorage. codeDiffBase = name;
                }
                if
                ( localStorage. nx !== undefined) { name = localStorage. nx; delete
                    localStorage. nx; localStorage. codeDiffNew = name;
                }
                if
                ( localStorage. bi !== undefined) { name = localStorage. bi; delete
                    localStorage. bi; localStorage. codeBeautify = name;
                }
                if
                ( localStorage. mi !== undefined) { name = localStorage. mi; delete
                    localStorage. mi; localStorage. codeMinify = name;
                }
                if
                ( localStorage. statdata !== undefined) { delete
                    localStorage. statdata;
                }
                if
                ( pd. stat. fdate === 0) { pd. stat. fdate = Date. now();
                }
                if
                ( pd. test. json === true) { if
                    ( localStorage. commentString !== undefined && localStorage. commentString !== null && localStorage. commentString !== "") { pd. commentString = JSON. parse( localStorage. commentString. replace( /api\./g,""));
                    }
                    if
                    ( localStorage. settings !== undefined && localStorage. settings !== null) { if
                        ( localStorage. settings. indexOf( ":undefined") > 0) { localStorage. settings = localStorage. settings. replace( /:undefined/g,":false");
                        }
                        pd. settings = JSON. parse( localStorage. settings); if
                        ( pd. settings. knownname === undefined || typeof pd. settings. knownname === "number") { pd. settings. knownname = "\"" + Math. random(). toString(). slice( 2) + Math. random(). toString(). slice( 2) + "\""; localStorage. settings = JSON. stringify( pd. settings);
                        }
                        if
                        ( pd. settings. diffreport !== undefined) { delete
                            pd. settings. diffreport; delete
                            pd. settings. beaureport; delete
                            pd. settings. minnreport; pd. settings. feedreport = {};pd. settings. codereport = {};
                        }
                    } else
                    if ( pd. settings. knownname === undefined || typeof pd. settings. knownname === "number") { pd. settings. knownname = "\"" + Math. random(). toString(). slice( 2) + Math. random(). toString(). slice( 2) + "\""; localStorage. settings = JSON. stringify( pd. settings);
                    }
                    if
                    ( localStorage. stat !== undefined) { if
                        ( statdump. length === 0) { pd. stat       = JSON. parse( localStorage. stat); pd. stat. visit = Number( pd. stat. visit) + 1; if
                            ( typeofpd. stat. fdate === "string") { pd. stat. fdate = Date. parse( pd. stat. fdate);
                            }
                            if
                            ( pd. stat. fdate === 0 || pd. stat. fdate === null || isNaN( pd. stat. fdate) === true) { pd. stat. fdate = Date. now();
                            }
                            a = ( Date. now() - pd. stat. fdate) / 86400000; if
                            ( a < 1) { a = 1;
                            }
                            pd. stat. avday  = Math. round( pd. stat. visit / a); pd. stat. useday = Math. round( pd. stat. usage / a);
                        }
                        node = pd. id( "stvisit"); if
                        ( node !== null) { node. innerHTML = pd. stat. visit;
                        }
                        node = pd. id( "stusage"); if
                        ( node !== null) { node. innerHTML = pd. stat. usage;
                        }
                        node = pd. id( "stuseday"); if
                        ( node !== null) { node. innerHTML = pd. stat. useday;
                        }
                        node = pd. id( "stfdate"); if
                        ( node !== null) { node. innerHTML = new Date( pd. stat. fdate). toLocaleDateString();
                        }
                        node = pd. id( "stavday"); if
                        ( node !== null) { node. innerHTML = pd. stat. avday;
                        }
                        node = pd. id( "stlarge"); if
                        ( node !== null) { node. innerHTML = pd. stat. large;
                        }
                        node = pd. id( "stdiff"); if
                        ( node !== null) { node. innerHTML = pd. stat. diff;
                        }
                        node = pd. id( "stbeau"); if
                        ( node !== null) { node. innerHTML = pd. stat. beau;
                        }
                        node = pd. id( "stminn"); if
                        ( node !== null) { node. innerHTML = pd. stat. minn;
                        }
                        node = pd. id( "stpars"); if
                        ( node !== null) { node. innerHTML = pd. stat. pars;
                        }
                        node = pd. id( "stmarkup"); if
                        ( node !== null) { node. innerHTML = pd. stat. markup;
                        }
                        node = pd. id( "stjs"); if
                        ( node !== null) { node. innerHTML = pd. stat. js;
                        }
                        node = pd. id( "stcss"); if
                        ( node !== null) { node. innerHTML = pd. stat. css;
                        }
                        node = pd. id( "stcsv"); if
                        ( node !== null) { node. innerHTML = pd. stat. csv;
                        }
                        node = pd. id( "sttext"); if
                        ( node !== null) { node. innerHTML = pd. stat. text;
                        }
                    }
                    localStorage. stat = JSON. stringify( pd. stat);
                }
                if
                ( statdump. length > 0) { delete
                    localStorage. statdata;
                }
            }
            if
            ( pd. test. agent. indexOf( "webkit") > 0 || pd. test. agent. indexOf( "blink") > 0) { inputs    = document. getElementsByTagName( "textarea"); inputsLen = inputs. length; for
                ( a = 0; a < inputsLen; a += 1) { inputs[a].removeAttribute( "wrap");
                }
            }
            if
            ( pd. o. codeBeauIn !== null) { pd. o. codeBeauIn. onkeyup = function dom__load_bindBeauInUp( e) { var
                    event = e || window. event; pd. recycle( event);
                };
                if
                ( pd. test. ace === true) { pd. o. codeBeauIn. onfocus   = textareafocus; pd. o. codeBeauIn. onblur    = textareablur; pd. o. codeBeauIn. onkeydown = function dom__load_bindBeauInDownCM( e) { var
                        event = e || window. event; pd. areaTabOut( event, this); pd. keydown( event);
                    };
                } else
                { pd. o. codeBeauIn. onfocus   = textareafocus; pd. o. codeBeauIn. onblur    = textareablur; pd. o. codeBeauIn. onkeydown = function dom__load_bindBeauInDown( e) { var
                        event = e || window. event; pd. fixtabs( event, pd. o. codeBeauIn); pd. areaTabOut( event, this); pd. keydown( event);
                    };
                }
            }
            if
            ( pd. o. codeBeauOut !== null && pd. test. ace === true) { pd. o. codeBeauOut. onfocus   = textareafocus; pd. o. codeBeauOut. onblur    = textareablur; pd. o. codeBeauOut. onkeydown = pd. areaTabOut;
            }
            if
            ( pd. o. codeMinnIn !== null) { pd. o. codeMinnIn. onkeyup = function dom__load_bindMinnInUp( e) { var
                    event = e || window. event; pd. recycle( event);
                };
                if
                ( pd. test. ace === true) { pd. o. codeMinnIn. onfocus   = textareafocus; pd. o. codeMinnIn. onblur    = textareablur; pd. o. codeMinnIn. onkeydown = function dom__load_bindMinnInDownCM( e) { var
                        event = e || window. event; pd. areaTabOut( event, this); pd. keydown( event);
                    };
                } else
                { pd. o. codeMinnIn. onfocus   = textareafocus; pd. o. codeMinnIn. onblur    = textareablur; pd. o. codeMinnIn. onkeydown = function dom__load_bindMinnInDown( e) { var
                        event = e || window. event; pd. fixtabs( event, this); pd. areaTabOut( event, this); pd. keydown( event);
                    };
                }
            }
            if
            ( pd. o. codeMinnOut !== null && pd. test. ace === true) { pd. o. codeMinnOut. onfocus   = textareafocus; pd. o. codeMinnOut. onblur    = textareablur; pd. o. codeMinnOut. onkeydown = pd. areaTabOut;
            }
            if
            ( pd. o. codeParsIn !== null) { pd. o. codeParsIn. onkeyup = function dom__load_bindParsInUp( e) { var
                    event = e || window. event; pd. recycle( event);
                };
                if
                ( pd. test. ace === true) { pd. o. codeParsIn. onfocus   = textareafocus; pd. o. codeParsIn. onblur    = textareablur; pd. o. codeParsIn. onkeydown = function dom__load_bindBeauInDownCM( e) { var
                        event = e || window. event; pd. areaTabOut( event, this); pd. keydown( event);
                    };
                } else
                { pd. o. codeParsIn. onfocus   = textareafocus; pd. o. codeParsIn. onblur    = textareablur; pd. o. codeParsIn. onkeydown = function dom__load_bindParsInDown( e) { var
                        event = e || window. event; pd. fixtabs( event, pd. o. codeParsIn); pd. areaTabOut( event, this); pd. keydown( event);
                    };
                }
            }
            if
            ( pd. o. codeParsOut !== null && pd. test. ace === true) { pd. o. codeParsOut. onfocus   = textareafocus; pd. o. codeParsOut. onblur    = textareablur; pd. o. codeParsOut. onkeydown = pd. areaTabOut;
            }
            if
            ( pd. o. codeDiffBase !== null) { if
                ( pd. test. ace === true) { pd. o. codeDiffBase. onkeyup   = function dom__load_bindAutoDiffBase() { pd. langkey( false, pd. ace. diffBase, "");
                    };
                    pd. o. codeDiffBase. onfocus   = textareafocus; pd. o. codeDiffBase. onblur    = textareablur; pd. o. codeDiffBase. onkeydown = pd. areaTabOut;
                } else
                { pd. o. codeDiffBase. onfocus   = textareafocus; pd. o. codeDiffBase. onblur    = textareablur; pd. o. codeDiffBase. onkeydown = function dom__load_bindDiffBaseDown( e) { var
                        event = e || window. event; pd. fixtabs( event, this); pd. areaTabOut( event, this);
                    };
                }
            }
            if
            ( pd. o. codeDiffNew !== null) { if
                ( pd. test. ace === true) { pd. o. codeDiffNew. onkeyup   = function dom__load_bindAutoDiffNew() { pd. langkey( false, pd. ace. diffNew, "");
                    };
                    pd. o. codeDiffNew. onfocus   = textareafocus; pd. o. codeDiffNew. onblur    = textareablur; pd. o. codeDiffNew. onkeydown = pd. areaTabOut;
                } else
                { pd. o. codeDiffNew. onfocus   = textareafocus; pd. o. codeDiffNew. onblur    = textareablur; pd. o. codeDiffNew. onkeydown = function dom__load_bindDiffNewDown( e) { var
                        event = e || window. event; pd. fixtabs( event, this); pd. areaTabOut( event, this);
                    };
                }
            }
            if
            ( pd. test. domain === false) { pd. o. report. feed. box. style. display = "none";
            } else
            if ( pd. o. report. feed. box !== null) { if
                ( pd. test. fs === true) { pd. o. report. feed. box. ondragover  = pd. filenull; pd. o. report. feed. box. ondragleave = pd. filenull; pd. o. report. feed. box. ondrop      = pd. filedrop;
                }
                pd. o. report. feed. body. onmousedown = top; title                             = pd. o. report. feed. box. getElementsByTagName( "h3")[0].getElementsByTagName( "button")[0];title. onmousedown                 = grab; title. ontouchstart                = grab; title. onfocus                     = headerfocus; title. onblur                      = headerblur; if
                ( pd. settings. feedreport === undefined) { pd. settings. feedreport = {};
                }
                if
                ( pd. settings. feedreport !== undefined && pd. settings. feedreport. min === false) { buttons               = pd. o. report. feed. box. getElementsByTagName( "p")[0];buttons. style. display = "block"; title. style. cursor    = "move"; if
                    ( buttons. innerHTML. indexOf( "save") > 0) { buttons. getElementsByTagName( "button")[1].innerHTML = "\u035f"; if
                        ( pd. test. agent. indexOf( "macintosh") > 0) { title. parentNode. style. width = ( (pd. settings. feedreport. width / 10) - 8.15) + "em";
                        } else
                        { title. parentNode. style. width = ( (pd. settings. feedreport. width / 10) - 9.75) + "em";
                        }
                    } else
                    { buttons. getElementsByTagName( "button")[0].innerHTML = "\u035f"; if
                        ( pd. test. agent. indexOf( "macintosh") > 0) { title. parentNode. style. width = ( (pd. settings. feedreport. width / 10) - 5.15) + "em";
                        } else
                        { title. parentNode. style. width = ( (pd. settings. feedreport. width / 10) - 6.75) + "em";
                        }
                    }
                    if
                    ( pd. settings. feedreport. top < 15) { pd. settings. feedreport. top = 15;
                    }
                    pd. o. report. feed. box. style. right    = "auto"; pd. o. report. feed. box. style. left     = ( pd. settings. feedreport. left / 10) + "em"; pd. o. report. feed. box. style. top      = ( pd. settings. feedreport. top / 10) + "em"; pd. o. report. feed. body. style. width   = ( pd. settings. feedreport. width / 10) + "em"; pd. o. report. feed. body. style. height  = ( pd. settings. feedreport. height / 10) + "em"; pd. o. report. feed. body. style. display = "block";
                }
                pd. id( "feedsubmit"). onclick = pd. feedsubmit;
            }
            if
            ( pd. o. report. code. box !== null) { if
                ( pd. test. fs === true) { pd. o. report. code. box. ondragover  = pd. filenull; pd. o. report. code. box. ondragleave = pd. filenull; pd. o. report. code. box. ondrop      = pd. filedrop;
                }
                pd. o. report. code. body. onmousedown = top; title                             = pd. o. report. code. box. getElementsByTagName( "h3")[0].getElementsByTagName( "button")[0];title. onmousedown                 = grab; title. ontouchstart                = grab; title. onfocus                     = headerfocus; title. onblur                      = headerblur; buttons                           = pd. o. report. code. box. getElementsByTagName( "p")[0];node                              = pd. id( "jsscope-yes"); if
                ( node !== null && node. checked === true && buttons. innerHTML. indexOf( "save") < 0) { if
                    ( pd. test. agent. indexOf( "firefox") > 0 || pd. test. agent. indexOf( "presto") > 0) { node = document. createElement( "a"); node. setAttribute( "href", "#"); node. onclick   = save; node. innerHTML = "<button class='save' title='Convert report to text that can be saved.' tabindex=" +              "'-1'>S</button>"; buttons. insertBefore( node, buttons. firstChild);
                    } else
                    { node = document. createElement( "button"); node. setAttribute( "class", "save"); node. setAttribute( "title", "Convert report to text that can be saved."); node. innerHTML = "S"; buttons. insertBefore( node, buttons. firstChild);
                    }
                }
                if
                ( pd. settings. codereport !== undefined && pd. settings. codereport. min === false) { buttons. style. display = "block"; title. style. cursor    = "move"; if
                    ( buttons. innerHTML. indexOf( "save") > 0) { buttons. getElementsByTagName( "button")[1].innerHTML = "\u035f"; if
                        ( pd. test. agent. indexOf( "macintosh") > 0) { title. parentNode. style. width = ( (pd. settings. codereport. width / 10) - 8.15) + "em";
                        } else
                        { title. parentNode. style. width = ( (pd. settings. codereport. width / 10) - 9.75) + "em";
                        }
                    } else
                    { buttons. getElementsByTagName( "button")[0].innerHTML = "\u035f"; if
                        ( pd. test. agent. indexOf( "macintosh") > 0) { title. parentNode. style. width = ( (pd. settings. codereport. width / 10) - 5.15) + "em";
                        } else
                        { title. parentNode. style. width = ( (pd. settings. codereport. width / 10) - 6.75) + "em";
                        }
                    }
                    if
                    ( pd. settings. codereport. top < 15) { pd. settings. codereport. top = 15;
                    }
                    pd. o. report. code. box. style. right    = "auto"; pd. o. report. code. box. style. left     = ( pd. settings. codereport. left / 10) + "em"; pd. o. report. code. box. style. top      = ( pd. settings. codereport. top / 10) + "em"; pd. o. report. code. body. style. width   = ( pd. settings. codereport. width / 10) + "em"; pd. o. report. code. body. style. height  = ( pd. settings. codereport. height / 10) + "em"; pd. o. report. code. body. style. display = "block";
                }
            }
            if
            ( pd. o. report. stat. box !== null) { if
                ( pd. test. fs === true) { pd. o. report. stat. box. ondragover  = pd. filenull; pd. o. report. stat. box. ondragleave = pd. filenull; pd. o. report. stat. box. ondrop      = pd. filedrop;
                }
                pd. o. report. stat. body. onmousedown = top; title                             = pd. o. report. stat. box. getElementsByTagName( "h3")[0].getElementsByTagName( "button")[0];title. onmousedown                 = grab; title. ontouchstart                = grab; title. onfocus                     = headerfocus; title. onblur                      = headerblur; if
                ( pd. settings. statreport !== undefined && pd. settings. statreport. min === false) { buttons               = pd. o. report. stat. box. getElementsByTagName( "p")[0];buttons. style. display = "block"; title. style. cursor    = "move"; if
                    ( buttons. innerHTML. indexOf( "save") > 0) { buttons. getElementsByTagName( "button")[1].innerHTML = "\u035f"; if
                        ( pd. test. agent. indexOf( "macintosh") > 0) { title. parentNode. style. width = ( (pd. settings. statreport. width / 10) - 8.15) + "em";
                        } else
                        { title. parentNode. style. width = ( (pd. settings. statreport. width / 10) - 6.75) + "em";
                        }
                    } else
                    { buttons. getElementsByTagName( "button")[0].innerHTML = "\u035f"; if
                        ( pd. test. agent. indexOf( "macintosh") > 0) { title. parentNode. style. width = ( (pd. settings. statreport. width / 10) - 5.15) + "em";
                        } else
                        { title. parentNode. style. width = ( (pd. settings. statreport. width / 10) - 6.75) + "em";
                        }
                    }
                    if
                    ( pd. settings. statreport. top < 15) { pd. settings. statreport. top = 15;
                    }
                    pd. o. report. stat. box. style. right    = "auto"; pd. o. report. stat. box. style. left     = ( pd. settings. statreport. left / 10) + "em"; pd. o. report. stat. box. style. top      = ( pd. settings. statreport. top / 10) + "em"; pd. o. report. stat. body. style. width   = ( pd. settings. statreport. width / 10) + "em"; pd. o. report. stat. body. style. height  = ( pd. settings. statreport. height / 10) + "em"; pd. o. report. stat. body. style. display = "block";
                }
            }
            inputs    = document. getElementsByTagName( "input"); inputsLen = inputs. length; for
            ( a = 0; a < inputsLen; a += 1) { type = inputs[a].getAttribute( "type"); id   = inputs[a].getAttribute( "id"); if
                ( type === "radio") { name = inputs[a].getAttribute( "name"); if
                    ( id === pd. settings[name]) { inputs[a].checked = true; if
                        ( name === "beauchar" || name === "diffchar") { pd. indentchar( inputs[a]);
                        }
                    }
                    if
                    ( id. indexOf( "feedradio") === 0) { inputs[a].onfocus                                               = feedradio; inputs[a].onblur                                               = feedblur; inputs[a].onclick                                              = feedradio; inputs[a].parentNode. getElementsByTagName( "label")[0].onclick = feedradio;
                    }
                    if
                    ( name === "mode") { inputs[a].onclick = pd. prettyvis; if
                        ( pd. settings. mode === id) { pd. prettyvis( inputs[a]);
                        } else
                        if ( pd. settings. mode === undefined) { pd. prettyvis( pd. o. modeDiff);
                        }
                    } else
                    if ( name === "diffchar") { inputs[a].onclick = pd. indentchar; if
                        ( pd. settings. diffchar === inputs[a].getAttribute( "id")) { inputs[a].checked = true; inputs[a].click();
                        } else
                        if ( pd. settings. diffchar !== undefined && pd. settings. diffchar !== "diff-space" && pd. settings. diffchar !== "diff-tab" && pd. settings. diffchar !== "diff-line") { node = pd. id( "diff-char"); if
                            ( node !== null) { node. value = pd. settings. diffchar;
                            }
                            node = pd. id( "diff-other"); if
                            ( node !== null) { node. checked = true; node. click();
                            }
                        }
                    } else
                    if ( name === "beauchar") { inputs[a].onclick = pd. indentchar; if
                        ( pd. settings. beauchar === inputs[a].getAttribute( "id")) { inputs[a].checked = true; inputs[a].click();
                        } else
                        if ( pd. settings. beauchar !== undefined && pd. settings. beauchar !== "beau-space" && pd. settings. beauchar !== "beau-tab" && pd. settings. beauchar !== "beau-line") { node = pd. id( "beau-char"); if
                            ( node !== null) { node. value = pd. settings. beauchar;
                            }
                            node = pd. id( "beau-other"); if
                            ( node !== null) { node. checked = true; node. click();
                            }
                        }
                    } else
                    if ( name === "jsscope") { inputs[a].onclick = hideBeauOut; if
                        ( id === "jsscope-yes" && inputs[a].checked === true) { pd. hideBeauOut( inputs[a]);
                        }
                    } else
                    if ( name === "ace-radio") { if
                        ( id === "ace-no" && inputs[a].checked === true && pd. test. ace === true) { acedisable( inputs[a]);
                        }
                        if
                        ( id === "ace-yes" && inputs[a].checked === true && pd. test. ace === false) { pd. id( "ace-no"). checked = true;
                        }
                ...
                if ( type === "text") { if
                    ( pd. test. ace === true && ( id === "diff-quan" || id === "beau-quan" || id === "minn-quan")) { inputs[a].onkeyup = pd. insize; if
                        ( pd. settings[id] !==undefined && pd. settings[id] !=="4" && isNaN( pd. settings[id]) === false) { if
                            ( id === "diff-quan") { if
                                ( pd. o. codeDiffBase !== null) { pd. ace. diffBase. getSession(). setTabSize( Number( pd. settings[id]));
                                }
                                if
                                ( pd. o. codeDiffNew !== null) { pd. ace. diffNew. getSession(). setTabSize( Number( pd. settings[id]));
                                }
                            } else
                            if ( id === "beau-quan") { if
                                ( pd. o. codeBeauIn !== null) { pd. ace. beauIn. getSession(). setTabSize( Number( pd. settings[id]));
                                }
                                if
                                ( pd. o. codeBeauOut !== null) { pd. ace. beauOut. getSession(). setTabSize( Number( pd. settings[id]));
                                }
                            } else
                            if ( id === "minn-quan") { if
                                ( pd. o. codeMinnIn !== null) { pd. ace. minnIn. getSession(). setTabSize( Number( pd. settings[id]));
                                }
                                if
                                ( pd. o. codeMinnOut !== null) { pd. ace. minnOut. getSession(). setTabSize( Number( pd. settings[id]));
                                }
                            } else
                            if ( id === "pars-quan") { if
                                ( pd. o. codeParsIn !== null) { pd. ace. parsIn. getSession(). setTabSize( Number( pd. settings[id]));
                                }
                                if
                                ( pd. o. codeParsOut !== null) { pd. ace. parsOut. getSession(). setTabSize( Number( pd. settings[id]));
                                }
                            }
                        }
                    } else
                    ...
                    if
                    ( pd. settings[id] !==undefined) { inputs[a].value = pd. settings[id];
                    }
                    if
                    ( id === "diff-char" || id === "beau-char") { inputs[a].onclick = pd. indentchar;
                    }
                } else
                if ( type === "file") { inputs[a].onchange = pd. file; inputs[a].onfocus = filefocus; inputs[a].onblur  = fileblur;
                }
            }
            node = pd. id( "ace-no"); if
            ( pd. test. ace === false && node !== null && node. checked === false) { node. checked = true;
            }
            inputs    = document. getElementsByTagName( "select"); inputsLen = inputs. length; for
            ( a = 0; a < inputsLen; a += 1) { id = inputs[a].getAttribute( "id"); if
                ( id === "colorScheme") { inputs[a].onchange = pd. colorScheme; if
                    ( pd. settings. colorScheme !== undefined) { inputs[a].selectedIndex = Number( pd. settings. colorScheme); pd. colorScheme( inputs[a]);
                    }
                } else
                if ( id === "language") { inputs[a].onchange = pd. codeOps; if
                    ( pd. settings. language !== undefined) { inputs[a].selectedIndex = Number( pd. settings. language); if
                        ( pd. o. lang[pd. o. lang. selectedIndex].value === "text" && pd. mode !== "diff") { inputs[a].selectedIndex = 0;
                        }
                    }
                } else
            ...
            inputs    = document. getElementsByTagName( "button"); inputsLen = inputs. length; for
            ( a = 0; a < inputsLen; a += 1) { name = inputs[a].getAttribute( "class"); id   = inputs[a].getAttribute( "id"); if
                ( name === null) { if
                    ( inputs[a].value === "Execute") { inputs[a].onclick = pd. recycle;
                    } else
                    if ( id === "resetOptions") { inputs[a].onclick = pd. reset;
                    }
                } else
                if ( name === "minimize") { inputs[a].onclick = pd. minimize;
                } else
                if ( name === "maximize") { inputs[a].onclick = pd. maximize; if
                    ( pd. settings[inputs[a].parentNode. parentNode. getAttribute( "id")] !==undefined && pd. settings[inputs[a].parentNode. parentNode. getAttribute( "id")].max === true) { inputs[a].click();
                    }
                } else
                if ( name === "resize") { inputs[a].onmousedown = resize;
                } else
                if ( name === "save") { node  = inputs[a];title = inputs[a].parentNode; if
                    ( title. nodeName. toLowerCase() === "a") { if
                        ( pd. test. agent. indexOf( "firefox") < 0 && pd. test. agent. indexOf( "presto") < 0) { buttons      = title. parentNode; node. onclick = save; title. removeChild( node); buttons. removeChild( title); buttons. insertBefore( node, buttons. firstChild); inputs[a].removeAttribute( "tabindex");
                        } else
                        { title. onclick = save;
                        }
                    } else
            ...
            if
            ( pd. o. save !== null) { pd. o. save. onclick = savecheck;
            }
            node = pd. id( "thirdparties"); if
            ( node !== null) { inputs    = node. getElementsByTagName( "a"); inputsLen = inputs. length; for
                ( a = 0; a < inputsLen; a += 1) { inputs[a].onclick = thirdparty;
                }
            }
            //webkit users get sucky textareas, because they refuse to
            //accept bugs related to long scrolling errors
            node = pd. id( "update"); if
            ( node !== null && typeof edition === "object") { node. innerHTML = ( functiondom__load_doc_conversion() { var
                    str   = String( edition. latest), list  = [
                            str. charAt( 0) + str. charAt( 1), Number( str. charAt( 2) + str. charAt( 3)), str. charAt( 4) + str. charAt( 5)
                        ],month = [
                            "January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"
                        ];list[1] -=1; if
                    ( list[2].charAt( 0) === "0") { list[2] =list[2].substr( 1);
                    }
                    return
                    "Updated: " + list[2] +" " + month[list[1]] +" 20" + list[0] +"<span>Version: <span>" + edition. version + "</span></span>";
                }());
            }
            if
            ( pd. o. comment !== null) { if
                ...
                if ( pd. commentString. length === 1) { pd. o. comment. innerHTML = "/*prettydiff.com " + pd. commentString[0].replace( "api.", "") + " */";
                } else
                { pd. o. comment. innerHTML = "/*prettydiff.com " + pd. commentString. join( ", "). replace( /api\./g,"") + " */";
                }
            }
            node = pd. id( "button-primary"); if
            ( node !== null) { node. onmouseover = pd. comment;
            }
            node = pd. id( "feedsubmit"); if
            ( node !== null) { node. onclick = pd. feedsubmit;
            }
            if
            ( typeoflocation === "object" && typeof location. href === "string" && location. href. indexOf( "?") > -1) { (
                functiondom__load_queryString() { var
                    b        = 0, c        = 0, color    = pd. id( "colorScheme"), colors   = ( color !== null)
                            ? color. getElementsByTagName( "option")
                            : [],options  = ( pd. o. lang !== null)
                            ? pd. o. lang. getElementsByTagName( "option")
                            : [],params   = location. href. split( "?")[1].split( "&"), paramLen = params. length, value    = "", source   = "", diff     = ""; for
                    ( b = 0; b < paramLen; b += 1) { if
                        ( params[b].indexOf( "m=") === 0) { value = params[b].toLowerCase(). substr( 2); if
                            ( value === "beautify" && pd. o. modeBeau !== null) { pd. prettyvis( pd. o. modeBeau); pd. o. modeBeau. checked = true;
                            } else
                            if ( value === "minify" && pd. o. modeMinn !== null) { pd. prettyvis( pd. o. modeMinn); pd. o. modeMinn. checked = true;
                            } else
                            if ( value === "diff" && pd. o. modeDiff !== null) { pd. prettyvis( pd. o. modeDiff); pd. o. modeDiff. checked = true;
                            } else
                            if ( value === "parse" && pd. o. modePars !== null) { pd. prettyvis( pd. o. modePars); pd. o. modePars. checked = true;
                            }
                        } else
                        if ( params[b].indexOf( "s=") === 0) { source = params[b].substr( 2);
                        } else
                        if ( params[b].indexOf( "d=") === 0 && pd. o. codeDiffNew !== null) { diff = params[b].substr( 2); if
                            ( pd. o. codeDiffNew !== null) { if
                                ( pd. test. ace === true) { pd. ace. diffNew. setValue( diff); pd. ace. diffNew. clearSelection();
                                } else
                                { pd. o. codeDiffNew. value = diff;
                                }
                            }
                        } else
                        if ( params[b].indexOf( "l=") === 0 && pd. o. lang !== null) { value = params[b].toLowerCase(). substr( 2); if
                            ( value === "text" || value === "plain" || value === "plaintext") { value = "text"; pd. prettyvis( pd. o. modeDiff);
                            } else
                            if ( value === "js" || value === "") { value = "javascript";
                            } else
                            if ( value === "markup") { value = "xml";
                            } else
                            if ( value === "jstl") { value = "jsp";
                            } else
                            if ( value === "erb" || value === "ejs") { value = "html_ruby";
                            } else
                            if ( value === "tss" || value === "titanium") { value = "tss";
                            }
                            for
                            ( c = options. length - 1; c > -1; c -= 1) { if
                                ( options[c].value === value) { pd. o. lang. selectedIndex = c; break;
                                }
                            }
                            if
                            ( pd. test. ace === true && c > -1) { pd. ace. diffBase. getSession(). setMode( "ace/mode/" + value); pd. ace. diffNew. getSession(). setMode( "ace/mode/" + value); pd. ace. beauIn. getSession(). setMode( "ace/mode/" + value); pd. ace. beauOut. getSession(). setMode( "ace/mode/" + value); pd. ace. minnIn. getSession(). setMode( "ace/mode/" + value); pd. ace. minnOut. getSession(). setMode( "ace/mode/" + value); pd. ace. parsIn. getSession(). setMode( "ace/mode/" + value); pd. ace. parsOut. getSession(). setMode( "ace/mode/" + value);
                            }
                            pd. codeOps( pd. o. lang);
                        } else
                        if ( params[b].indexOf( "c=") === 0) { value = params[b].toLowerCase(). substr( 2); for
                            ( c = colors. length - 1; c > -1; c -= 1) { if
                                ( colors[c].innerHTML. toLowerCase() === value) { color. selectedIndex = c; pd. colorScheme( color); break;
                                }
                            }
                        } else
                        if ( params[b].indexOf( "jsscope") === 0) { if
                            ( pd. o. jsscope !== null) { pd. o. jsscope. checked = true;
                            }
                            pd. hideBeauOut( pd. o. jsscope);
                        } else
                        if ( params[b].indexOf( "jscorrect") === 0) { node = pd. id( "jscorrect-yes"); if
                            ( node !== null) { node. checked = true;
                            }
                        } else
                        if ( params[b].indexOf( "html") === 0) { node = pd. id( "html-yes"); if
                            ( node !== null) { pd. options( node);
                            }
                            node = pd. id( "htmld-yes"); if
                            ( node !== null) { pd. options( node);
                            }
                            value = params[b].toLowerCase(). substr( 2); for
                            ( c = options. length - 1; c > -1; c -= 1) { if
                                ( options[c].value === "html") { pd. o. lang. selectedIndex = c; pd. codeOps( pd. o. lang); break;
                                }
                            }
                            if
                            ( pd. test. ace === true) { pd. ace. diffBase. getSession(). setMode( "ace/mode/html"); pd. ace. diffNew. getSession(). setMode( "ace/mode/html"); pd. ace. beauIn. getSession(). setMode( "ace/mode/html"); pd. ace. beauOut. getSession(). setMode( "ace/mode/html"); pd. ace. minnIn. getSession(). setMode( "ace/mode/html"); pd. ace. minnOut. getSession(). setMode( "ace/mode/html"); pd. ace. parsIn. getSession(). setMode( "ace/mode/html"); pd. ace. parsOut. getSession(). setMode( "ace/mode/html");
                            }
                        }
                    }
                    if
                    ( source !== "") { if
                        ( pd. o. codeBeauIn !== null && pd. mode === "beau") { if
                            ( pd. test. ace === true) { pd. ace. beauIn. setValue( source); pd. ace. beauIn. clearSelection();
                            } else
                            { pd. o. codeBeauIn. value = source;
                            }
                            pd. recycle();
                        } else
                        if ( pd. o. codeMinnIn !== null && pd. mode === "minn") { if
                            ( pd. test. ace === true) { pd. ace. minnIn. setValue( source); pd. ace. minnIn. clearSelection();
                            } else
                            { pd. o. codeMinnIn. value = source;
                            }
                            pd. recycle();
                        } else
                        if ( pd. o. codeParsIn !== null && pd. mode === "pars") { if
                            ( pd. test. ace === true) { pd. ace. parsIn. setValue( source); pd. ace. parsIn. clearSelection();
                            } else
                            { pd. o. codeParsIn. value = source;
                            }
                            pd. recycle();
                        } else
                        if ( pd. o. codeDiffBase !== null && pd. mode === "diff") { if
                            ( pd. test. ace === true) { pd. ace. diffBase. setValue( source); pd. ace. diffBase. clearSelection();
                            } else
            ...
            if
            ( pd. test. ace === true) { node = pd. id( "minn-quan"); if
                ( node !== null) { node. parentNode. parentNode. style. display = "block";
                }
                node = pd. id( "minn-space"); if
                ( node !== null) { node. parentNode. parentNode. style. display = "block";
                }
            }
            if
            ...
            if
            ( pd. settings. feedback. newb === false && pd. stat. usage > 2 && pd. stat. visit < 5 && pd. test. domain === true && pd. o. report. feed. box !== null) { pd. settings. feedback. newb = true; node                      = pd. id( "feedintro"); if
                ( node !== null) { node. innerHTML = "Thank you for trying Pretty Diff. Please let me know what you think of this tool" + ".";
                }
                if
                ( pd. test. json === true && pd. test. ls === true) { localStorage. settings = JSON. stringify( pd. settings);
                }
            ...
            if
            ( pd. settings. feedback. veteran === false && pd. stat. usage > 2500 && pd. test. domain === true && pd. o. report. feed. box !== null) { pd. settings. feedback. veteran = true; if
                ( node !== null) { node. innerHTML = "Thank you for the loyal and frequent use of this tool. Would you mind sparing a " + "few seconds on a brief survey?";
                }
                if
                ( pd. test. json === true && pd. test. ls === true) { localStorage. settings = JSON. stringify( pd. settings);
                }
            ...
            if
            ( pd. test. audio !== null) { pd. audio        = {};pd. audio. binary = window. atob( "SUQzBAAAAAAAFlRFTkMAAAAMAAADTGF2ZjUyLjMxLjD/+9RkAA/wAABpAAAACAAADSAAAAEAAAGkAAAA"+ "IAAANIAAAARMQU1FMy45OSAoYWxwaGEpVQA9wAChVSyrkSX4folg8BYiJjMKc1SuX4pepo4ETrVZqIv8"
                        ...
                        + "6wrRGLJCsIU5TEFNRTMuOTkgKGFscGhhKao="); pd. audio. play   = function dom__load_audio() { var
                    source  = pd. test. audio. createBufferSource(), buff    = new ArrayBuffer( pd. audio. binary. length), bytes   = new Uint8Array( buff), z       = 0, bytelen = buff. byteLength; for
                    ( z = 0; z < bytelen; z += 1) { bytes[z] =pd. audio. binary. charCodeAt( z);
                    }
                    pd. test. audio. decodeAudioData( buff, function dom__load_audio_decode( buffer) { source. buffer = buffer; source. loop   = false; source. connect( pd. test. audio. destination); source. start( 0, 0, 1.8);
                    });
                };
            }
        }
        if
        ( page === "doc") { (
            functiondom__load_doc() { var
                b             = 0, componentArea = {},row           = [],dateCell      = {},dateList      = [],output        = [],rowLen        = 0, date          = 0, lib           = "", colorParam    = ( typeoflocation === "object" && typeof location. href === "string" && location. href. indexOf( "?") > -1)
                        ? location. href. toLowerCase(). split( "?")[1]
                        :"", conversion    = function dom__load_doc_conversion( dateInstance) { var
                        str   = String( dateInstance), list  = [
                                str. charAt( 0) + str. charAt( 1), Number( str. charAt( 2) + str. charAt( 3)), str. charAt( 4) + str. charAt( 5)
                            ],month = [
                                "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"
                            ];list[1] -=1; return
                        list[2] +" " + month[list[1]] +" 20" + list[0];
                    };
                    node = pd. id( "colorScheme"); if
                ( node !== null) { if
                    ( localStorage !== null && localStorage. settings !== undefined && localStorage. settings !== null && localStorage. settings. indexOf( ":undefined") > 0) { localStorage. settings = localStorage. settings. replace( /:undefined/g,":false");
                    }
                    pd. settings = ( pd. test. ls === true && pd. test. json === true && localStorage. settings !== undefined)
                        ? JSON. parse( localStorage. settings)
                        : {};if
                    ( colorParam. indexOf( "c=") === 0 || colorParam. indexOf( "&c=") > -1) { if
                        ( colorParam. indexOf( "&c=") > -1) { colorParam. substr( colorParam. indexOf( "&c=") + 1);
                        }
                        colorParam = colorParam. split( "&")[0];colorParam = colorParam. substr( colorParam. indexOf( "=") + 1); row        = node. getElementsByTagName( "option"); rowLen     = row. length; for
                        ( b = 0; b < rowLen; b += 1) { if
                            ( row[b].value. toLowerCase() === colorParam) { node. selectedIndex = b; break;
                            }
                        }
                    }
                    if
                    ( ((rowLen > 0 && b !== rowLen) || rowLen === 0) && pd. settings. colorScheme !== undefined) { node. selectedIndex = pd. settings. colorScheme;
                    }
                    pd. colorScheme( node); node. onchange = pd. colorScheme;
                }
                componentArea = pd. id( "components"); if
                ( componentArea !== null && typeof edition === "object") { componentArea = componentArea. getElementsByTagName( "tbody")[0];row           = componentArea. getElementsByTagName( "tr"); rowLen        = row. length; for
                    ( b = 0; b < rowLen; b += 1) { dateCell = row[b].getElementsByTagName( "td")[3];lib      = row[b].getElementsByTagName( "a")[0].innerHTML; if
                        ( lib === "charDecoder.js") { date               = edition. charDecoder; dateCell. innerHTML = conversion( date); dateList. push( [
                                date, row[b].innerHTML
                            ]);
                        } else
                        if ( lib === "csspretty.js") { date               = edition. csspretty; dateCell. innerHTML = conversion( date); dateList. push( [
                                date, row[b].innerHTML
                            ]);
                        } else
                        if ( lib === "csvbeauty.js") { date               = edition. csvbeauty; dateCell. innerHTML = conversion( date); dateList. push( [
                                date, row[b].innerHTML
                            ]);
                        } else
                        if ( lib === "csvmin.js") { date               = edition. csvmin; dateCell. innerHTML = conversion( date); dateList. push( [
                                date, row[b].innerHTML
                            ]);
                        } else
                        if ( lib === "diffview.css") { date               = edition. css; dateCell. innerHTML = conversion( date); dateList. push( [
                                date, row[b].innerHTML
                            ]);
                        } else
                        if ( lib === "diffview.js") { date               = edition. diffview; dateCell. innerHTML = conversion( date); dateList. push( [
                                date, row[b].innerHTML
                            ]);
                        } else
                        if ( lib === "documentation.xhtml") { date               = edition. documentation; dateCell. innerHTML = conversion( date); dateList. push( [
                                date, row[b].innerHTML
                            ]);
                        } else
                        if ( lib === "dom.js") { date               = edition. api. dom; dateCell. innerHTML = conversion( date); dateList. push( [
                                date, row[b].innerHTML
                            ]);
                        } else
                        if ( lib === "jspretty.js") { date               = edition. jspretty; dateCell. innerHTML = conversion( date); dateList. push( [
                                date, row[b].innerHTML
                            ]);
                        } else
                        if ( lib === "markup_beauty.js") { date               = edition. markup_beauty; dateCell. innerHTML = conversion( date); dateList. push( [
                                date, row[b].innerHTML
                            ]);
                        } else
                        if ( lib === "markupmin.js") { date               = edition. markupmin; dateCell. innerHTML = conversion( date); dateList. push( [
                                date, row[b].innerHTML
                            ]);
                        } else
                        if ( lib === "node-local.js") { date               = edition. api. nodeLocal; dateCell. innerHTML = conversion( date); dateList. push( [
                                date, row[b].innerHTML
                            ]);
                        } else
                        if ( lib === "prettydiff.com.xhtml") { date               = edition. webtool; dateCell. innerHTML = conversion( date); dateList. push( [
                                date, row[b].innerHTML
                            ]);
                        } else
                        if ( lib === "prettydiff.js") { date               = edition. prettydiff; dateCell. innerHTML = conversion( date); dateList. push( [
                                date, row[b].innerHTML
                            ]);
                        } else
                        if ( lib === "prettydiff.wsf") { date               = edition. api. wsh; dateCell. innerHTML = conversion( date); dateList. push( [
                                date, row[b].innerHTML
                            ]);
                        } else
                        if ( lib === "ace") { date               = edition. addon. ace; dateCell. innerHTML = conversion( date); dateList. push( [
                                date, row[b].innerHTML
                            ]);
                        }
                    }
                    rowLen   = dateList. length; dateList = dateList. sort( functiondom__load_sort_forward( componentArea, row) { return
                        componentArea[1] ===row[1];
                    }). reverse(). sort( functiondom__load_sort_reverse( componentArea, row) { return
                        componentArea[0] -row[0];
                    }); for
                    ( b = dateList. length - 1; b > -1; b -= 1) { output. push( "<tr>"); output. push( dateList[b][1]); output. push( "</tr>");
                    }
                    componentArea. innerHTML = output. join( "");
                }
                (
                functiondom__foldSearch() { var
                    div   = document. getElementsByTagName( "div"), len   = div. length, inca  = 0, incb  = 0, ol    = [],li    = [],lilen = 0; for
                    ( inca = 0; inca < len; inca += 1) { if
                        ( div[inca].getAttribute( "class") === "beautify") { ol = div[inca].getElementsByTagName( "ol"); if
                            ( ol[0].getAttribute( "class") === "count") { li    = ol[0].getElementsByTagName( "li"); lilen = li. length; for
                                ( incb = 0; incb < lilen; incb += 1) { if
                                    ( li[incb].getAttribute( "class") === "fold") { li[incb].onmousedown = pd. beaufold;
                                    }
                                }
                            }
                        }
