        ...
        var a              = 0,
            ...
            depthPush      = function jspretty__tokenize_depthPush() {
                ...
                if ((token[last] === "{" || token[last] === "x{") && ((wordx === "else" && token[last] !== "if") || wordx === "do" || wordx === "try" || wordx === "finally" || wordx === "switch")) {
                    ...
                    if (classy[classy.length - 1] === 0 && wordx !== "return") {
                        ...
                        //public class {
                        depth.push(["function", last]);
                    } else if (depthlist.length > 0 && token[last - 1] !== ":" && depthlist[depthlist.length - 1][0] === "object" && depth[last - 1] !== undefined && (token[depth[last - 1][1] - 2] === "{" || token[depth[last - 1][1] - 2] === ",")) {
            ...
            asi            = function jspretty__tokenize_asi(isEnd) {
                var len   = token.length - 1,
                    ...
                    paren = false,
                    opers = false,
                    funce = (isEnd === true && (deepl[0] === "function" || (tokel === "}" && depthlist.length > 0 && depthlist[depthlist.length - 1][0] === "function")) && (types[deepl[1] - 1] === "operator" || types[deepl[1] - 1] === "start")),
            ...
            commentSplit   = function jspretty__tokenize_commentSplit(item) {
                ...
                if (spacely === true) {
                    ...
                    do {
                        ...
                        ltoke = "//" + item.slice(0, start).replace(/(\s+)$/, "");
                        tokenpush(false, 0);
                        item  = item.slice(start + 1);
                        start = options.wrap - 2;
            ...
            var a           = 0,
                ...
                tern        = function jspretty__algorithm_tern(x) {
                    var aa = 0,
                        bb = depth[a][1],
                        lasttest = false;
                    if (ternary.length < 1 || ternary[ternary.length - 1] < depth[a][1]) {
                        return;
                    }
