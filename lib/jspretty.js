    ...
    jspretty = function jspretty(args) {
            ...
            var a = 0,
                ...
                word = function jspretty__algorithm_word() {
                    ...
                    if (a - 1 !== lines[l][0] && ltoke === "}" && varline.length > 1 && varline[varline.length - 1] === true && ctoke !== "else" && ctoke !== "while" && ctoke !== "catch") {
                        ...
                        varline[varline.length - 1] = false;
                        t.push(a);
                    } else if (a - 1 === lines[l][0] && ctoke !== "var" && ((varline[varline.length - 1] === true && ltoke === "}") || (ltype !== "method" && ltype !== "separator" && ltype !== "operator" && ltype !== "start" && ltoke !== "}"))) {
                        ltoke = ";";
                        ltype = "separator";
                        if (ltoke !== "}") {
                            l += 1;
                    ...
                    if (ctoke === "new" && a !== b - 1 && !(token[a + 1] === "Date" || token[a + 1] === "RegExp" || token[a + 1] === "DataView" || token[a + 1] === "ArrayBuffer" || token[a + 1] === "Proxy" || token[a + 1] === "ParallelArray" || token[a + 1] === "Int8Array" || token[a + 1] === "Uint8Array" || token[a + 1] === "Int16Array" || token[a + 1] === "Uint16Array" || token[a + 1] === "Int32Array" || token[a + 1] === "Uint32Array" || token[a + 1] === "Float32Array" || token[a + 1] === "Float64Array")) {
                        u.push(a);
                    }
        ...
        result = (function jspretty__result() {
            ...
            if (scope === true) {
                ...
                data.push("</ol>");
                c = [
                    "<p>Scope analysis only covers <strong>function</strong> scope and does not analyze global scope.</p>", "<p><em>", t.length, "</em> instances of <strong>missing semicolons</strong> counted.</p>", "<p><em>", u.length, "</em> unnecessary instances of the keyword <strong>new</strong> counted.</p>", data.join(""), f
                ];
                summary = c.join("");
                data = [];
                c = [];
