    ...
    (function jspretty__tokenize() {
        var a              = 0,
            ...
            asi            = function jspretty__tokenize_asi(isEnd) {
                ...
                if (tokel === ";" || tokel === "," || (tokel !== undefined && tokel.slice(0, 2) === "{%") || (next === ";" && isEnd === false)) {
                    return;
                }
            ...
            tname = function jspretty__tokenize_tname(x) {
                var sn = 2,
                    en = 0,
                    name = "",
                    namelist = ["autoescape", "block", "capture", "case", "comment", "embed", "filter", "for", "form", "if", "macro", "raw", "sandbox", "spaceless", "tablerow", "unless", "verbatim"];
                if ((/\s/).test(x.charAt(2)) === true) {
                    do {
                        sn += 1;
                    } while ((/\s/).test(x.charAt(sn)) === true);
                }
                en = sn;
                do {
                    en += 1;
                } while ((/\s/).test(x.charAt(en)) === false);
                name = x.slice(sn, en);
