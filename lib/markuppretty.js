...
var markuppretty = function markuppretty_(options) {
                    ...
                    if (comment === true) {
                        ...
                        if (quote === "") {
                            ...
                            if (stest === true && (/\s/).test(b[a]) === false && b[a] !== lastchar) {
                                ...
                                //opening quote
                                quote = b[a];
                            } else if (comment === false && b[a] === "<" && b[a + 1] === "!" && b[a + 2] === "-" && b[a + 3] === "-" && b[a + 4] !== "#" && types[types.length - 1] !== "conditional") {
                                quote = "-->";
    ...
    output = (function markuppretty__apply() {
        ...
        for (a = 0; a < c; a += 1) {
            ...
            } else if (options.unformatted === false && options.mode === "beautify" && presv[a] === false && (attrs[a].length > 0 || (wrap > 0 && token[a].length > wrap)) && (types[a] === "content" || types[a] === "start" || types[a] === "singleton" || types[a] === "template_start" || types[a] === "template") && (types[a] !== "singleton" || token[a].charAt(0) !== "{")) {
                wrapper();
