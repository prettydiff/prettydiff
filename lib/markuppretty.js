...
var markuppretty = function markuppretty_(options) {
                ...
                script       = function markuppretty__beautify_script(twig) {
                    ...
                    if (bracetest === true) {
                        if (lines[a] === 0) {
                            ...
                            token[a] = token[a].replace(/^(\s+)/, jsxstart) + tabs + "}";
                        }
                    }
                    if (cdataS !== "") {
    ...
    output = (function markuppretty__apply() {
        ...
        for (a = 0; a < c; a += 1) {
            ...
            } else if (options.unformatted === false && options.mode === "beautify" && (attrs[a].length > 0 || (wrap > 0 && token[a].length > wrap)) && (types[a] === "content" || types[a] === "start" || types[a] === "singleton" || types[a] === "template_start" || types[a] === "template") && (types[a] !== "singleton" || token[a].charAt(0) !== "{")) {
                wrapper();
            } else if (options.unformatted === false && attrs[a].length > 0) {
