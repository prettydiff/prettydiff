...
var markuppretty = function markuppretty_(options) {
    ...
    (function markuppretty__options() {
        ...
        wrap           = (isNaN(options.wrap) === true || options.jsx === true || options.mode === "diff" || options.textpreserve === true)
            ? 0
            : Number(options.wrap);
    }());
    //type definitions:
        ...
        var a        = 0,
            ...
            tag      = function markuppretty__tokenize_tag(end) {
                ...
                for (a = a; a < c; a += 1) {
                    ...
                    if (quote === "") {
                        ...
                        if (stest === true && (/\s/).test(b[a]) === false && b[a] !== lastchar) {
                            ...
                            //opening quote
                            quote = b[a];
                        } else if (output[0] !== "{" && b[a] === "{" && (options.dustjs === true || b[a + 1] === "{" || b[a + 1] === "%" || b[a + 1] === "@" || b[a + 1] === "#")) { //opening embedded template expression
                            if (b[a + 1] === "{") {
                                if (b[a + 2] === "{") {
                                    quote = "}}}";
                                } else {
                        ...
                        } else if (b[a] === lastchar && (options.jsx === false || jsxcount === 0)) {
                ...
                if (attrs[attrs.length - 1].length > 0) {
                    ...
                    if (jscom[jscom.length - 1] === false && options.jsx === false) {
                ...
                if (options.tagsort === true && types[types.length - 1] === "end" && types[types.length - 2] !== "start") {
                    (function markuppretty__tokenize_tag_sorttag() {
                        ...
                        lines         = lines.slice(0, startStore)
                            .concat(store.lines);
                        presv         = presv.slice(0, startStore)
                            .concat(store.presv);
            ...
            content  = function markuppretty__tokenize_content() {
                ...
                for (a = a; a < c; a += 1) {
                    ...
                    } else if (b[a] === "<" || (b[a] === "[" && b[a + 1] === "%") || (b[a] === "{" && (options.jsx === true || options.dustjs === true || b[a + 1] === "{" || b[a + 1] === "%" || b[a + 1] === "@" || b[a + 1] === "#"))) {
        ...
        (function markuppretty__beautify() {
            var a            = 0,
                ...
                tab          = (function markuppretty__beautify_tab() {
                    ...
                    var b = 0;
                    indent -= 1;
                    if (ltype === "start" || (options.jsx === true && (/^\s+\{/).test(token[a - 1]) === true && lines[a] === 0)) {
                        return level.push("x");
                    }
                ...
                content      = function markuppretty__beautify_content() {
                    var b = 0;
                    if (lines[a] === 0 && options.force_indent === false) {
    ...
    return (function markuppretty__apply() {
        var a            = 0,
            ...
            wrapper         = function markuppretty__apply_wrapper() {
                ...
                if (list.length > 0) {
                    ...
                    len      = list.length;
                    for (b = 0; b < len; b += 1) {
                        if (list[b].length + wordslen + 1 > wrap) {
                            nl(lev + 1, content);
                            wordslen = 0;
                        } else {
                            content.push(" ");
        ...
        for (a = 0; a < c; a += 1) {
            ...
            if (jscom[a] === true) {
                attrcom();
            } else if (((types[a] === "content" && wrap > 0 && token[a].length > wrap) || attrs[a].length > 0) && options.mode === "beautify") {
                wrapper();
            ...
            if (types[a] === "singleton" && options.spaceclose === true) {
                token[a] = token[a].replace(/(\/>)$/, " />");
            }
            if (token[a] !== "</prettydiffli>") {
                    ...
                    accessibility = (function markuppretty__apply_summary_accessibility() {
                        var findings   = [],
                            tagsbyname = function markuppretty__apply_summary_accessibility_tagsbyname() {
                                ...
                                //violations - number of accessibility violations
                                //count
                                data.violations = 0;
                                //badnest - checks for improperly orderd tags
                                //[tagname, start index, end index]
                                data.badnest = [];
                                //obsoleteTags - checks for obsolete or presentation only tag names of start and singleton tags
                                //token index
                                data.obsoleteTags = [];
                                ...
                                data.tabindex = [];
                                //htmllang - if there is an <html> tag does it contain a lang or xml:lang attribute?
                                //boolean
                                data.htmllang = false;
                                ...
                                for (b = 0; b < c; b += 1) {
                                    ...
                                    if ((types[b] === "start" || types[b] === "singleton") && (tagname === "font" || tagname === "center" || tagname === "basefont" || tagname === "b" || tagname === "i" || tagname === "u" || tagname === "small" || tagname === "big" || tagname === "blink" || tagname === "plaintext" || tagname === "spacer" || tagname === "strike" || tagname === "tt" || tagname === "xmp")) {
                                        ...
                                        if (types[b] === "start" && headtest.test(tagname) === true) {
                                            ...
                                            if (data.headings.length > 0 && z - data.headings[data.headings.length - 1][1] > 1) {
                                                data.violations += 1;
                                                data.headings.push([b, z, true]);
                                            } else {
                                                data.headings.push([b, z, false]);
                                            }
                                        }
                                        y = attrs[b].length;
                                        for (x = 0; x < y; x += 1) {
                                            ...
                                            if (data.obsoleteTags[data.obsoleteTags.length - 1] !== b && (attr[0] === "alink" || attr[0] === "align" || attr[0] === "background" || attr[0] === "border" || attr[0] === "color" || attr[0] === "compact" || attr[0] === "face" || attr[0] === "height" || attr[0] === "language" || attr[0] === "link" || (attr[0] === "name" && tagname !== "meta" && tagname !== "iframe" && tagname !== "select" && tagname !== "input" && tagname !== "textarea") || attr[0] === "nowrap" || attr[0] === "size" || attr[0] === "start" || attr[0] === "text" || (attr[0] === "type" && tagname !== "script" && tagname !== "style" && tagname !== "input" && tagname !== "button" && tagname !== "link") || (attr[0] === "value" && tagname !== "input" && tagname !== "option" && tagname !== "textarea" && tagname !== "button") || attr[0] === "version" || attr[0] === "vlink" || attr[0] === "width")) {
                                                data.obsoleteAttr.push([b, x]);
                                            }
                                            if (attr[0] === "tabindex") {
                                                if (isNaN(Number(attr[1])) === true || Number(attr[1]) > 0) {
                                                    data.tabindex.push([b, true]);
                                                    data.violations += 1;
                                                } else {
                                                    data.tabindex.push([b, false]);
                                                }
                                            }
                                        }
                                        if (fortest === true) {
                                    ...
                                    if (types[b] === "start" || types[b] === "template_start") {
                                        ...
                                        if (tagname !== "/" + namestack[namestack.length - 1][0]) {
                                            namestack[namestack.length - 1].push(b);
                                            data.badnest.push(namestack[namestack.length - 1]);
                                        }
                                        namestack.pop();
                                    }
                                }
                                ...
                                attr.push("<div class='a11y'>");
                                attr.push("<div>");
                                if (data.htmllang === false) {
                                    data.violations += 1;
                                    attr.push("<h4>HTML tag is a <strong>missing</strong> lang or xml:lang attribute</strong></h4>");
                                } else {
                                    attr.push("<h4>HTML lang or xml:lang attribute is present</h4>");
                                }
                                attr.push("<p>The lang attribute ensures the natural language is properly understood by assisting applications.</p>");
                                attr.push("</div><div>");
                                ...
                                if (b > 0) {
                                    ...
                                    attr.push(" containing obsolete or inappropriate attributes</h4> <p>Obsolete attributes do not appropriately describe content.</p> <ol>");
                                    for (x = 0; x < b; x += 1) {
                                        tagname = token[data.obsoleteAttr[x][0]].replace(/&/g, "&amp;")
                                            .replace(/</g, "&lt;")
                                            .replace(/>/g, "&gt;")
                                ...
                                for (x = 0; x < b; x += 1) {
                                    ...
                                    if (y < 0) {
                                        data.formNoLabel.push(formID[x]);
                                    }
                                }
                                ...
                                if (b > 0) {
                                    ...
                                    attr.push(" violation");
                                    attr.push("</h4> <p>The tabindex attribute should have a" +
                                            " 0 or -1 value and should not be over used. Only tabindexes with a value greater than 0 are counted as violations, but every element with a tabindex attribute is listed to quickly indicate if it used excessively.</p> <ol>");
                                    for (x = 0; x < b; x += 1) {
                                        attr.push("<li><code>");
                                ...
                                if (b > 0) {
                                    ...
                                    attr.push("<p>When heading tags are present it is important they are properly ordered so that the content they describe can be navigated in the proper order.</p>");
                                }
                                attr.push("</div><div>");
                                ...
                                if (b > 0) {
                                    ...
                                    attr.push(" have an empty <em>alt</em> attribute value</h4> <p>Empty alt text is not necessarily a violation, such as the case of tracking pixels. If an image has embedded text this content should be supplied in the alt attribute.</p>");
                                    for (x = 0; x < b; x += 1) {
                                        attr.push("<li><code>");
                                    ...
                                    attr.push("<p>Empty alt text is not necessarily a violation, such as the case of tracking pixels. If an image has embedded text this content should be supplied in the alt attribute.</p>");
                                }
                                attr.push("</div>");
                                attr.push("</div>");
