...
var markuppretty = function markuppretty_(options) {
            ...
            tag      = function markuppretty__tokenize_tag(end) {
                    ...
                    if (comment === true) {
                        ...
                        if (quote === "") {
                            ...
                            if (stest === true && (/\s/).test(b[a]) === false && b[a] !== lastchar) {
                                ...
                                quote = "-->";
                            } else if (lexer[0] !== "{" && b[a] === "{" && end !== "%>" && end !== "%]" && (options.dustjs === true || b[a + 1] === "{" || b[a + 1] === "%" || b[a + 1] === "@" || b[a + 1] === "#")) {
                                //opening embedded template expression
                                if (b[a + 1] === "{") {
                                    if (b[a + 2] === "{") {
                                        quote = "}}}";
                            ...
                            } else if (b[a] === lastchar && (lexer.length > end.length + 1 || lexer[0] === "]") && (options.jsx === false || jsxcount === 0)) {
                ...
                cheat = (function markuppretty__tokenize_tag_cheat() {
                    ...
                    if (options.html === true) {
                        ...
                        if (options.jsx === false) {
                            if (tname === "li") {
                                if (litag === list && (list !== 0 || (list === 0 && types[types.length - 1].indexOf("template") < 0))) {
                ...
                if (types[e].indexOf("template") > -1) {
                    if (element.slice(0, 2) === "{%") {
                        lexer = ["autoescape", "block", "capture", "case", "comment", "embed", "filter", "for", "form", "if", "macro", "paginate", "raw", "sandbox", "spaceless", "tablerow", "unless", "verbatim"];
            ...
            content  = function markuppretty__tokenize_content() {
                ...
                for (a = a; a < c; a += 1) {
                    ...
                    if (ext === true) {
                        ...
                        } else if (quote === b[a] && (quote === "\"" || quote === "'" || quote === "`") && esctest() === false) {
                            quote = "";
    ...
    if (global.meta.error === "" || global.meta.error === undefined) {
        global.meta.error = (function () {
            ...
            if (startend > 0) {
                ...
                return error + " than end tags!";
            } else if (startend < 0) {
                error = startend + " more end tag";
                if (startend > 1) {
                    error = error + "s";
                }
                return error + " than start tags!";
            } else {
                return "";
            }
        }());
        ...
        return (function markuppretty__parse() {
            var a        = 0,
                ...
                def      = {
                    ...
                    types: "string - Data types of the tokens: cdata, comment, conditional, content, end, ignore, linepreserve, script, sgml, singleton, start, template, template_else, template_end, template_start, xml",
                    ...
                    linen: "number - The line number in the original source where the token started, which is used for reporting and analysis",
                    jscom: "boolean - Whether the token is a JavaScript comment if in JSX format",
                    presv: "boolean - Whether the token is preserved verbatim as found.  Useful for comments and HTML 'pre' tags."
                },
            ...
            if (options.parseFormat === "sequential") {
                for (a = 0; a < c; a += 1) {
                    record.push([token[a], types[a], attrs[a], lines[a], linen[a], jscom[a], presv[a]]);
                }
                return {
                    data      : record,
                    definition: def
            ...
            if (options.parseFormat === "htmltable") {
                return (function csspretty__parse_html() {
                    ...
                    report.push("<th>attrs</th>");
                    report.push("<th>lines</th>");
                    report.push("<th>linen</th>");
                    report.push("<th>jscom</th>");
                    report.push("<th>presv</th>");
    ...
    if (options.mode === "beautify" || options.mode === "diff" || options.mode === "analysis") {
            ...
            var a            = 0,
                ...
                script       = function markuppretty__beautify_script(twig) {
                    var list      = [],
                        ...
                        mode      = options.mode;
                    if (twig === true) {
                        token[a] = token[a].replace(twigStart, "").replace(twigEnd, "");
                    }
                    stats.script[0] += 1;
                    ...
                    } else if ((bracetest === true || twig === true) && lines[a] === 0) {
                        level.push("x");
                        types[a] = "singleton";
                    } else {
                        level.push(0);
    ...
    if (token.length === 0) {
        ...
        return global.meta.error;
    }

    output = (function markuppretty__apply() {
        var a            = 0,
            c            = level.length,
            build        = [],
            ...
            wrapper      = function markuppretty__apply_wrapper() {
                ...
                if (list.length > 0) {
                    ...
                    ind = (function markuppretty__apply_wrapper_tab() {
                        var atline = lf,
                            atnum  = lev + 1;
                        do {
                            atline = atline + tab;
                    ...
                    if (wrap === 0 || len <= wrap || tname === "cfset"|| tname === "cfreturn" || tname === "cfif" || tname === "cfelseif") {
                        if (presv[a] === true) {
                            ...
                            xlen     = (string === "/>")
                                ...
                                : 2;
                            if (options.force_attribute === true) {
                                token[a] = (token[a].slice(0, token[a].length - xlen) + ind + atty + string);
                            } else {
                    ...
                    list = token[a].replace(/\s+/g, " ").split(" ");
                    len  = list.length;
            ...
            var len           = token.length,
                ...
                parseErrors   = (function markuppretty__apply_summary_parseErrors() {
                    var x     = parseError.length,
                        ...
                        fails = [];
                    data.violations += x;
                    global.meta.error = parseError[0].replace(binfix, "");
                    if (x === 0) {
                        return "";
                    }
                ...
                statistics    = (function markuppretty__apply_summary_statistics() {
                    var stat       = [],
                        ...
                        rowBuilder = function markuppretty__apply_summary_statistics_rowBuilder(type) {
                            ...
                            stat.push("</th><td");
                            if ((global.meta.error.indexOf(" more start tag") || global.meta.error.indexOf(" more end tag")) && (type === "start" || type === "end")) {
                                stat.push(" class=\"bad\"");
                            }
                            stat.push(">");
                            stat.push(itema);
