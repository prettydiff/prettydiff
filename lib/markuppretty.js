...
(function markuppretty_init() {
    ...
    var markuppretty = function markuppretty_(options) {
        ...
        output = (function markuppretty__beautify() {
            ...
            if (options.mode !== "minify") {
                for (a = 0; a < c; a += 1) {
                    if (twigStart.test(token[a]) === true && twigEnd.test(token[a]) === true && tagName(token[a - 1]) !== "script" && tagName(token[a - 1]) !== "style" && (/\D-+\D/).test(token[a]) === false && (/^(\{%\s*((comment)|(else))\s*)/).test(token[a]) === false) {
                        script(true);
                    }
                    if (types[a] === "start") {
                        level.push(indent);
