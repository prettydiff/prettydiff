            ...
            tag      = function markuppretty__tokenize_tag(end) {
                    ...
                    if (comment === true) {
                        ...
                        if (b[a] === lastchar && output.length > end.length + 1) {
                            ...
                            if (e < 0) {
                                if (end === "endcomment") {
                                    f -= 1;
                                    if ((/\s/).test(output[f]) === true) {
                                        do {
                                            f -= 1;
                                        } while ((/\s/).test(output[f]) === true);
                                    }
                                    if (output[f - 1] === "{" && output[f] === "%") {
                                        end = "%}";
                                        lastchar = "}";
                                    }
                                } else {
                        ...
                        if (quote === "") {
                            ...
                            if (stest === true && (/\s/).test(b[a]) === false && b[a] !== lastchar) {
                                ...
                                if (output[0] === "{" && output[1] === "%" && output.join("").replace(/\s+/g, "") === "{%comment%}") {
                                    end = "endcomment";
                                    lastchar = "t";
                                    preserve = true;
                                    comment = true;
                ...
                if (types[e].indexOf("template") > -1) {
                    if (element.slice(0, 2) === "{%") {
                        output = ["autoescape", "block", "capture", "case", "comment", "embed", "filter", "for", "form", "if", "macro", "paginate", "raw", "sandbox", "spaceless", "tablerow", "unless", "verbatim"];
                        if (tname === "else" || tname === "elseif" || tname === "when") {
                            types[e] = "template_else";
                        } else {
                            for (f = output.length - 1; f > -1; f -= 1) {
                                if (tname === output[f]) {
                                    types[e] = "template_start";
                                    break;
                                }
                                if (tname === "end" + output[f]) {
                        ...
                        if ((/^(\{\{\s*end\s*\}\})$/).test(element) === true) {
                            types[e] = "template_end";
                        } else if (tname === "block" || tname === "define" || tname === "if" || tname === "range" || tname === "with") {
                            types[e] = "template_start";
                        }
