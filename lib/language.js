                ...
                var d               = 0,
                    ...
                    javascriptA     = function language_auto_notmarkup_javascriptA() {
                        if (sample.indexOf("(") > -1 || sample.indexOf("=") > -1 || (sample.indexOf(";") > -1 && sample.indexOf("{") > -1)) {
                            if ((/((public)|(private))\s+/).test(sample) === true || (/\w<\w+(,\s+\w+)*>/).test(sample) === true) {
                                if ((/:\s*((number)|(string))/).test(sample) === false && ((/((final\s+static)|(static\s+void))/).test(sample) === true || (/((public)|(private))\s+class/).test(sample) === true)) {
                                    return output("java");
                                }
