...
(function language_init() {
    ...
    language.auto = function language_auto(sample, defaultLang) {
        ...
        if ((/\sclass\s+\w/).test(sample) === false && (/(\s|;|\})((if)|(for)|(function\s*\w*))\s*\(/).test(sample) === false && vartest === false && (/return\s*\w*\s*(;|\})/).test(sample) === false && (sample === undefined || (/^(\s*#(?!(!\/)))/).test(sample) === true || (/\n\s*(\.|@)\w+(\(?|(\s*:))/).test(sample) === true)) {
            return cssA();
        }
        b = sample
