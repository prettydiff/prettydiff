    ...
    var diffview = function diffview_(options) {
        (function diffview__options() {
            ...
            options.diffview = (options.diffview === "inline")
                ? "inline"
                : "sidebyside";
            options.diffcomments = (options.diffcomments === true || options.diffcomments === "true");
            ...
            options.semicolon = (options.semicolon === true || options.semicolon === "true");
                        ...
                        if (prior !== undefined) {
                            ...
                            if (prior[0] === "insert" && prior[4] - prior[3] === 1) {
                                ...
                                if (code[0] === "delete") {
                                    ...
                                    if (one[code[2] - 1] === two[prior[4] - 1] || one[code[2]] === two[prior[4] - 1]) {
                                        if (prior[2] - prior[1] > 1) {
                                            prior[2] = prior[2] - 1;
                                            prior[4] = prior[4] - 11;
                                    ...
                                    if (one[code[2] - 1] === two[prior[4] - 1] || one[code[2] - 1] === two[prior[4]]) {
        ...
        return (function diffview__report() {
            ...
            for (a = 0; a < opcodesLength; a = a + 1) {
                ...
                if (foldstart > -1 && options.diffline === false) {
                ...
                if (options.diffcli === true) {
                    ...
                    if (options.diffspaceignore === true && change === "replace" && baseTextArray[baseStart] !== undefined && newTextArray[newStart] !== undefined && baseTextArray[baseStart].replace(/\s+/g, "") === newTextArray[newStart].replace(/\s+/g, "")) {
                        ...
                        if (a > 0 && opcodes[a - 1][0] === "equal") {
                            foldcount = options.context + 1;
                            if ((ntest === true || change === "insert") && (options.diffspaceignore === false || (/^(\s+)$/g).test(newTextArray[newStart]) === false)) {
                                foldstart = foldstart + 1;
                                data[0].push("");
                                if (a > 0) {
                                    data[0].push("\u001b[36mLine: " + (opcodes[a - 1][2]) + "\u001b[39m");
                                } else {
                                    data[0].push("\u001b[36mLine: 0\u001b[39m");
                                }
                                if (foldcount > 0) {
                                    do {
                                        if (baseEnd - foldcount > 0) {
                                            data[0].push(newTextArray[newEnd - foldcount]);
                                        }
                                ...
                                foldstart = foldstart + 1;
                                data[0].push("");
                                data[0].push("\u001b[36mLine: " + (baseStart + 1) + "\u001b[39m");
                                if (foldcount > 0) {
                                    do {
                                        if (baseEnd - foldcount > 0) {
                                            data[0].push(baseTextArray[baseEnd - foldcount]);
                                        }
                                        foldcount = foldcount - 1;
                                    } while (foldcount > 1);
                                }
                        ...
                        if ((ntest === true || change === "insert") && (options.diffspaceignore === false || (/^(\s+)$/g).test(newTextArray[newStart]) === false)) {
                            do {
                                data[0].push("\u001b[32m" + newTextArray[newStart + foldcount] + "\u001b[39m");
                                foldcount = foldcount + 1;
                            ...
                            do {
                                data[0].push("\u001b[31m" + baseTextArray[baseStart + foldcount] + "\u001b[39m");
                                foldcount = foldcount + 1;
                            ...
                            do {
                                ...
                                data[0].push("\u001b[31m" + charcompOutput[0].replace(/<pd>/g, "\u001b[1m").replace(/<\/pd>/g, "\u001b[0m\u001b[31m") + "\u001b[39m");
                                data[0].push("\u001b[32m" + charcompOutput[1].replace(/<pd>/g, "\u001b[1m").replace(/<\/pd>/g, "\u001b[0m\u001b[32m") + "\u001b[39m");
                                foldcount = foldcount + 1;
                            } while (foldcount < 7 && foldcount + baseStart < baseEnd);
                        }
                        if (((change === "insert" && foldcount + newStart === newEnd) || (change !== "insert" && foldcount + baseStart === baseEnd)) && options.context > 0 && a< opcodesLength - 1 && opcodes[a + 1][0] === "equal") {
                            foldcount = 0;
                            baseStart = opcodes[a + 1][1];
                            baseEnd   = opcodes[a + 1][2] - baseStart;
                            do {
                                data[0].push(baseTextArray[baseStart + foldcount]);
                                foldcount = foldcount + 1;
