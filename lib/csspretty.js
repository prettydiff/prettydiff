...
var csspretty = function csspretty_(options) {
    ...
    (function csspretty__tokenize() {
        var a          = 0,
            ...
            spacer     = function csspretty__tokenize_space(end) {
                var slen = space.split(lf)
                    .length;
                if (slen > 1 && end === true && options.preserve === true) {
                    endline = true;
                    return;
                }
                ...
                var aa    = token.length,
                    ...
                    value = function csspretty__tokenize_item_value(val) {
                        var x      = val.split(""),
                            ...
                            block  = "",
                            values = [],
                            qchar  = (options.quoteConvert === "none" || options.quoteConvert === "double")
                                ? "\""
                                : "'";
                        ...
                        for (cc = 0; cc < leng; cc += 1) {
                            if (options.noleadzero === false && (/^(\.\d)/).test(values[cc]) === true) {
                                ...
                                if (values[cc].charAt(4) !== "\"") {
                                    if (values[cc].charAt(4) === "'" && qchar === "\"") {
                                        values[cc] = values[cc].replace("url('", "url(\"");
                                    } else {
                                        values[cc] = values[cc].replace("url(", "url(" + qchar);
                                        if (values[cc] === "url(" + qchar + ")") {
                                            values[cc] = "url(" + qchar + qchar + ")";
                                        }
                                    }
                                }
                                if (values[cc].charAt(values[cc].length - 2) !== qchar) {
                                    if (values[cc].charAt(values[cc].length - 2) === "'" || values[cc].charAt(values[cc].length - 2) === "\"") {
                                        values[cc] = values[cc].substr(0, values[cc].length - 2);
                                    } else {
                                        values[cc] = values[cc].substr(0, values[cc].length - 1);
                                    }
                                    values[cc] = values[cc] + qchar + ")";
                                }
                            ...
                            } else if ((qchar === "\"" && values[cc].charAt(0) === "'" && values[cc].charAt(values[cc].length - 1) === "'") || (qchar === "'" && values[cc].charAt(0) === "\"" && values[cc].charAt(values[cc].length - 1) === "\"")) {
            ...
            comment    = function csspretty__tokenize_comment(inline) {
                var aa        = 0,
                    ...
                    spareLine = [];
                spacer(false);
                type = (inline === true && lines[lines.length - 1] === 0 && types[types.length - 1] !== "comment" && types[types.length - 1] !== "comment-inline")
                    ? "comment-inline"
                    : "comment";
    ...
    if (options.mode !== "minify") {
        output = (function csspretty__beautify() {
            ...
            for (a = 0; a < len; a += 1) {
                if (types[a] === "start") {
                    if (a > 0 && token[a - 1].charAt(token[a - 1].length - 1) === "#") {
                        ...
                        build.push(token[a]);
                        indent += 1;
                        if (types[a + 1] !== "selector" || options.cssinsertlines === false) {
                            nl(indent);
                        }
                    }
                } else if (types[a] === "end") {
                    if (mixin === true) {
                        ...
                        nl(indent);
                        build.push(token[a]);
                        if (options.cssinsertlines === true && types[a + 1] === "selector" && lines[a] < 2 && token[a - 1] !== "{") {
                            build.push(lf);
                        } else if (types[a + 1] !== "end" && types[a + 1] !== "semi") {
                            if (options.cssinsertlines === true && types[a + 1] === "comment" && types[a] === "end") {
                                build.push(lf);
                            } else {
                                nl(indent);
                            }
                    ...
                    if (a > 0 && types[a - 1] !== "comment" && options.cssinsertlines === true) {
                        nl(indent);
                    }
                    ...
                    if (types[a] === "comment" && types[a - 1] === "comment-inline") {
                        build.push(lf);
                    }
                    if (lines[a - 1] > 0) {
                        nl(indent);
            ...
            if (options.preserve === true && (lines[lines.length - 1] > 0 || endline === true)) {
                return build.join("")
                    .replace(/(\s+)$/, lf);
            }
            return build.join("")
