...
var csspretty = function csspretty_(options) {
    ...
    (function csspretty__tokenize() {
        var a          = 0,
            ...
            spacer     = function csspretty__tokenize_space(end) {
                var slen  = space.split(lf).length,
                    value = 0;
                if (slen > 2 && options.preserve === true) {
                    value = 2;
                } else if (slen > 1) {
                    value = 1;
                } else if (types[types.length - 1] === "comment") {
                ...
                var aa    = token.length,
                    ...
                    value = function csspretty__tokenize_item_value(val) {
                        ...
                        for (cc = 0; cc < leng; cc += 1) {
                            ...
                            } else if (qchar !== "" && ((qchar === "\"" && values[cc].charAt(0) === "'" && values[cc].charAt(values[cc].length - 1) === "'") || (qchar === "'" && values[cc].charAt(0) === "\"" && values[cc].charAt(values[cc].length - 1) === "\""))) {
                                ...
                                if (token[set[aa][2]] !== undefined && token[set[aa][0]].indexOf(name) === 0) {
                                    ...
                                    if (token[set[aa][0]] === name || token[set[aa][0]].indexOf(name + " ") === 0) {
                                        ...
                                        if (valsplit.length === 1) {
                                            value = [
                                                token[set[aa][2]], token[set[aa][2]], token[set[aa][2]], token[set[aa][2]]
                                            ];
    ...
    if (options.mode !== "minify") {
        output = (function csspretty__beautify() {
            ...
            //beautification loop
            for (a = 0; a < len; a += 1) {
                if (lines[a] > 1 && (types[a] === "start" || types[a] === "end" || types[a] === "selector" || types[a] === "comment")) {
                    b = build.length;
                    do {
                        b -= 1;
                        if (build[b] === lf) {
                ...
                if (types[a] === "start") {
                    if (a > 0 && token[a - 1].charAt(token[a - 1].length - 1) === "#") {
                        ...
                        build.push(token[a]);
                        indent += 1;
                        if (types[a + 1] !== "end" && (types[a + 1] !== "selector" || options.cssinsertlines === false)) {
                            nl(indent);
                        }
                    }
                } else if (types[a] === "end") {
                    if (mixin === true) {
                        ...
                        if (options.cssinsertlines === true && types[a + 1] === "selector" && lines[a] < 2 && token[a - 1] !== "{") {
                            ...
                            if (options.cssinsertlines === true) {
                                build.push(lf);
                            } else {
                                nl(indent);
                            }
                    ...
                    if (types[a + 1] === "comment-inline") {
                        build.push(" ");
                    } else if (types[a + 1] !== "end" && types[a + 1] !== "comment") {
                        nl(indent);
                    }
                    ...
                    if (a > 0 && types[a - 1] !== "start" && build[build.length - 1] !== tab && types[a] !== "comment-inline") {
                        nl(indent);
                    }
                    build.push(token[a]);
