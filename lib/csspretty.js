...
var csspretty = function csspretty_(options) {
    ...
    if (typeof options.source !== "string" || options.source === "" || (/^(\s+)$/).test(options.source) === true) {
        ...
        global.meta.error = options.source;
        ...
        var a          = 0,
            ...
            objSort    = function csspretty__tokenize_objSort() {
                ...
                for (cc = startlen; cc > -1; cc -= 1) {
                    ...
                    if (dd < 0 && cc < startlen) {
                        if (keys.length > 1 && (types[cc - 1] === "selector" || types[cc - 1] === "propvar" || (types[cc - 2] ===  "propvar" && types[cc - 1] === "value") || token[cc - 1] === "=" || token[cc - 1] === ":" || token[cc - 1] === "[" || token[cc - 1] === "{" || token[cc - 1] === "," || cc === 0)) {
                            keys.sort(sort);
                            keylen   = keys.length;
                            semiTest = false;
            ...
            buildtoken = function csspretty__tokenize_build() {
                ...
                if (token[token.length - 1].indexOf("extend(") === 0) {
                    ...
                    if (types[types.length - 1] !== "semi" && (types[types.length - 2] === "property" || types[types.length - 2] === "propvar")) {
                        ltype = "value";
                        types.push("value");
                    } else {
                        ltype = "propvar";
            ...
            item       = function csspretty__tokenize_item(type) {
                ...
                if (ltype === "item" && types[aa].indexOf("external") < 0) {
                    if (type === "start") {
                        ...
                        if (token[aa].indexOf("(") < 0 || token[aa].indexOf("(") > token[aa].indexOf(",") || token[aa].indexOf(")") < token[aa].indexOf(",")) {
                            token[aa] = token[aa]
                                .split(",")
                                .sort()
                                .join(",");
                        ...
                        if (options.mode === "beautify" || options.mode === "parse" || (options.mode === "diff" && options.diffcomm === true)) {
            ...
            comment    = function csspretty__tokenize_comment(inline) {
                ...
                if (options.mode === "beautify" || options.mode === "parse" || (options.mode === "diff" && options.diffcomm === true) || (options.mode === "minify" && options.topcoms === true)) {
    ...
    if (options.mode === "parse") {
        return (function csspretty__parse() {
            ...
            if (options.parseFormat === "sequential") {
                for (a = 0; a < c; a += 1) {
                    record.push([token[a], types[a], lines[a]]);
                }
                if (options.nodeasync === true) {
                    return [{
                        data      : record,
