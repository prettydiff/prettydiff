                        ...
                        modifyFile = function node_apps_build_libraries_modifyFile(file:string, fileFlag:string):void {
                            node.fs.readFile(file, "utf8", function node_apps_build_libraries_modifyFile(err:Error, data:string):void {
                                ...
                                if (fileFlag === "documentation") {
                                    ...
                                    modify({
                                        end: "// diffview insertion end",
                                        injectFlag: `diffview=${diffview}`,
                                        start: "// diffview insertion start"
                                    });
                                        ...
                                        callback: function node_apps_readmethod_resolve_stat_resolveItem_directoryCallback(list:directoryList):void {
                                            const source:string = options.source.slice(0, options.source.lastIndexOf(sep) + 1),
                                                ...
                                                out:string = node.path.resolve(options.output),plural:string = (listlen === 1)
                                                    ? ""
                                                    : "s",
                                                address = function node_apps_readmethod_resolve_stat_resolveItem_directoryCallback_address(item:string):string {
                                                            ...
                                                            apps.readFile({
                                                                callback: function node_apps_readmethod_resolve_stat_resolveItem_directoryCallback_readFiles_callback(args:readFile, dump:string|Buffer):void {
                                                                    if (typeof dump === "string") {
                                                                        ...
                                                                        writeFile(address(args.path), result);
                                                                    } else {
                                                                        writeFile(address(args.path), dump);
                                                                    }
                                                                },
                                                                index: a,
                                                                path: list[a][0],
                                                ...
                                                makedir = function node_apps_readmethod_resolve_stat_resolveItem_directoryCallback_makedir():void {
                                                    do {
                                                        if (list[a][1] === "directory") {
                                                            apps.makedir(address(list[a][0]), function node_apps_readmethod_resolve_stat_resolveItem_directoryCallback_makedir_callback():void {
                                                                if (a < listlen) {
